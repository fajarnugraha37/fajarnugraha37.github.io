...
**Updated:** 7/15/2026 13:50:40  
**Exported:** 7/15/2026 14:41:41  
...
## Response:
7/15/2026, 1:44:15 PM

Berikut **cheat sheet Java gRPC Server** untuk konteks microservice Java 17+, Maven, Jersey/JAX-RS, MyBatis, PostgreSQL, Kubernetes.

> Catatan penting: gRPC Java server **tidak annotation-heavy seperti JAX-RS**. Mayoritas konfigurasi dilakukan lewat `.proto`, generated class, builder API, interceptor, dan runtime config. Generated Java code membuat class service dengan suffix `Grpc`, dan server implementer biasanya extend inner abstract class `ServiceNameImplBase`. ([gRPC](https://grpc.io/docs/languages/java/generated-code/))

---

# 1. Core Dependency Cheat Sheet
...
<dependencies>
    ...
    <!-- Netty transport, commonly used for server -->
    <dependency>
        <groupId>io.grpc</groupId>
        <artifactId>grpc-netty-shaded</artifactId>
    </dependency>

    <!-- Health/reflection/support services -->
    <dependency>
        <groupId>io.grpc</groupId>
        <artifactId>grpc-services</artifactId>
    </dependency>

    <!-- Rich error model: google.rpc.Status, BadRequest, RetryInfo, etc. -->
    <dependency>
    ...
    <!-- Often needed for generated code on modern JDKs -->
    <dependency>
        <groupId>javax.annotation</groupId>
        <artifactId>javax.annotation-api</artifactId>
        <version>1.3.2</version>
...
# 2. Proto Options yang Paling Penting untuk Java Server
...
| Option / Element | Fungsi | Praktik |
|---|---|---|
| `syntax = "proto3"` | Versi protobuf syntax | Umum untuk gRPC modern |
| `package` | Namespace protobuf | Pakai domain/capability/version |
| `java_package` | Package Java generated code | Wajib eksplisit |
| `java_multiple_files = true` | Generate top-level Java classes | Biasanya disarankan |
| `java_outer_classname` | Wrapper outer class | Wajib eksplisit agar stabil |
| `service` | Kontrak service gRPC | Boundary capability |
| `rpc` | Method RPC | Nama operation/business intent |
| `message` | Request/response schema | Jangan bocorkan DB entity |
| `enum` | Nilai typed | Mulai dari `UNSPECIFIED = 0` |

Generated Java class service memakai nama service + suffix `Grpc`; package Java diambil dari `java_package`, dan jika `java_package` tidak ada, generated code memakai proto package-hal ini sebaiknya dihindari karena proto package biasanya bukan reverse-domain Java package. ([gRPC](https://grpc.io/docs/languages/java/generated-code/))

---

# 3. Annotation Cheat Sheet

## 3.1 gRPC-Specific / Generated Annotations

| Annotation | Package | Dipakai di Mana | Catatan |
|---|---|---|---|
| `@GrpcGenerated` | `io.grpc.stub.annotations.GrpcGenerated` | Generated outer class/stub tertentu | Untuk membantu annotation processor; user **tidak perlu** menaruh ini di class sendiri |
| `@ExperimentalApi` | `io.grpc.ExperimentalApi` | API gRPC tertentu | API bisa berubah; jangan asal pakai untuk library production reusable |
| `@Internal` | `io.grpc.Internal` | API internal gRPC | Jangan pakai di application code |
| `@Generated` | `javax.annotation.Generated` / variasi generated code | Generated source | Bisa butuh dependency `javax.annotation-api` di Java modern |
| `@Override` | Java standard | Implementasi method RPC | Wajib secara praktik saat override method generated |
| `@Deprecated` | Java standard | API lama/deprecated | Perhatikan jika contoh lama memakai API deprecated |

`@GrpcGenerated` memang digunakan oleh stub compiler untuk menandai generated outer classes, tetapi dokumentasinya menyebut user tidak perlu memberi annotation ini pada class sendiri dan tidak semua stub harus memilikinya. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/stub/annotations/GrpcGenerated.html?utm_source=chatgpt.com))

## 3.2 Annotation yang Bukan gRPC Core, Tapi Sering Dipakai

| Annotation | Sumber | Fungsi |
|---|---|---|
| `@ApplicationScoped` | CDI/Weld | Lifecycle singleton service |
| `@Inject` | CDI/Jakarta | Dependency injection |
| `@Singleton` | javax/jakarta/inject atau DI framework | Singleton service |
| `@Named` | CDI/Jakarta | Qualifier sederhana |
| `@PostConstruct` | Jakarta annotation | Start lifecycle |
| `@PreDestroy` | Jakarta annotation | Shutdown lifecycle |
| `@Provider` | JAX-RS | Untuk REST exception mapper/filter, bukan gRPC |
| `@Path`, `@GET`, `@POST` | JAX-RS | REST boundary, bukan gRPC |
| `@Slf4j` | Lombok | Logging helper, bukan gRPC |
| `@VisibleForTesting` | Guava/others | Test visibility marker |

Penting: **method gRPC server tidak diberi annotation seperti `@GET`/`@POST`**. Method server muncul dari `.proto` dan di-override dari generated `ImplBase`.

---

# 4. Generated Server Class Cheat Sheet
...
Generated Java biasanya punya:

| Generated Class / Member | Fungsi |
|---|---|
| `CaseQueryServiceGrpc` | Outer generated gRPC service class |
| `CaseQueryServiceGrpc.CaseQueryServiceImplBase` | Base class untuk server implementation |
| `CaseQueryServiceGrpc.CaseQueryServiceStub` | Async client stub |
| `CaseQueryServiceGrpc.CaseQueryServiceBlockingStub` | Blocking client stub |
| `CaseQueryServiceGrpc.CaseQueryServiceFutureStub` | Future unary client stub |
| `bindService()` | Mengikat service implementation ke `ServerServiceDefinition` |
| `getServiceDescriptor()` | Descriptor service |
| `getGetCaseMethod()` | Descriptor method tertentu |
| `newStub(Channel)` | Membuat async stub |
| `newBlockingStub(Channel)` | Membuat blocking stub |
| `newFutureStub(Channel)` | Membuat future stub |

Generated service implementation base class memakai suffix `ImplBase`; jika method tidak dioverride, default-nya mengembalikan error unimplemented ke client. ([gRPC](https://grpc.io/docs/languages/java/generated-code/))

---

# 5. RPC Method Signature Cheat Sheet

| RPC Type | Proto | Java Server Signature |
|---|---|---|
| Unary | `rpc GetCase(GetCaseRequest) returns (GetCaseResponse);` | `void getCase(GetCaseRequest request, StreamObserver<GetCaseResponse> responseObserver)` |
| Server streaming | `rpc StreamEvents(Request) returns (stream Event);` | `void streamEvents(Request request, StreamObserver<Event> responseObserver)` |
| Client streaming | `rpc Upload(stream Chunk) returns (UploadResponse);` | `StreamObserver<Chunk> upload(StreamObserver<UploadResponse> responseObserver)` |
| Bidirectional streaming | `rpc Chat(stream ChatIn) returns (stream ChatOut);` | `StreamObserver<ChatIn> chat(StreamObserver<ChatOut> responseObserver)` |

gRPC Java generated-code reference menyebut unary dan server-streaming menerima single request + `StreamObserver<Response>`, sedangkan client-streaming dan bidirectional-streaming mengembalikan `StreamObserver<Request>`. ([gRPC](https://grpc.io/docs/languages/java/generated-code/))

---

# 6. Minimal Server Implementation

```java id="qfj787"
public final class CaseQueryGrpcService
        extends CaseQueryServiceGrpc.CaseQueryServiceImplBase {

    private final CaseApplicationService applicationService;
    private final CaseGrpcMapper mapper;
    private final GrpcServerErrorMapper errorMapper;

    public CaseQueryGrpcService(
            CaseApplicationService applicationService,
            CaseGrpcMapper mapper,
            GrpcServerErrorMapper errorMapper
    ) {
    ...
    public void getCase(
        ...
        try {
            CaseView view = applicationService.getCaseById(request.getCaseId());
...
| Praktik | Catatan |
|---|---|
| Always `@Override` | Hindari salah signature |
| Handler tipis | Validate, map, call app service, map response/error |
| Jangan query MyBatis langsung di handler | Tetap lewat application/domain layer |
| Jangan expose exception mentah | Pakai `Status` / error mapper |
| Unary success | `onNext(response)` lalu `onCompleted()` |
| Failure | `onError(statusException)` |

---

# 7. `StreamObserver` Cheat Sheet

| Method | Fungsi | Rule Penting |
|---|---|---|
| `onNext(V value)` | Kirim/terima message | Unary response maksimal sekali |
| `onError(Throwable t)` | Akhiri stream dengan error | Harus jadi call terakhir |
| `onCompleted()` | Akhiri stream sukses | Harus jadi call terakhir |

`StreamObserver` dipakai oleh client stub dan service implementation untuk semua jenis method, termasuk unary; implementasi tidak wajib thread-safe, dan jika beberapa thread menulis ke observer yang sama, aplikasi harus melakukan sinkronisasi sendiri. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/stub/StreamObserver.html))
...
responseObserver.onCompleted(); // SALAH: onError harus terminal
```

---

# 8. Server Bootstrap Class Cheat Sheet

## 8.1 `io.grpc.ServerBuilder`

| Method | Fungsi |
|---|---|
| `ServerBuilder.forPort(int port)` | Membuat builder untuk port |
| `addService(BindableService)` | Register service implementation |
| `addService(ServerServiceDefinition)` | Register service definition manual |
| `intercept(ServerInterceptor)` | Interceptor global untuk semua service |
| `executor(Executor)` | Custom executor |
| `directExecutor()` | Jalankan app code di transport thread; **hindari jika blocking** |
| `maxInboundMessageSize(int bytes)` | Limit request message size |
| `maxInboundMetadataSize(int bytes)` | Limit metadata/header size |
| `useTransportSecurity(File certChain, File privateKey)` | TLS server |
| `handshakeTimeout(long, TimeUnit)` | Timeout TLS/connection handshake |
| `keepAliveTime(long, TimeUnit)` | Server ping behavior |
| `keepAliveTimeout(long, TimeUnit)` | Timeout menunggu ping response |
| `permitKeepAliveTime(long, TimeUnit)` | Batas agresivitas keepalive client |
| `permitKeepAliveWithoutCalls(boolean)` | Izinkan ping tanpa active RPC |
| `maxConnectionIdle(long, TimeUnit)` | Terminate idle connection |
| `maxConnectionAge(long, TimeUnit)` | Terminate connection setelah umur tertentu |
| `maxConnectionAgeGrace(long, TimeUnit)` | Grace period sebelum connection ditutup |
| `addTransportFilter(ServerTransportFilter)` | Hook transport-level, advanced |
| `addStreamTracerFactory(ServerStreamTracer.Factory)` | Hook observability traffic, advanced |
| `build()` | Build `Server` |

`ServerBuilder` menyediakan `addService`, `intercept`, `executor`, `directExecutor`, max inbound message/metadata size, keepalive, TLS, dan connection management methods. `directExecutor()` dapat meningkatkan performa, tetapi aplikasi tidak boleh blocking jika memakainya; default executor tanpa custom executor adalah static cached thread pool, dan custom executor tidak otomatis dimiliki/shutdown oleh server. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/ServerBuilder.html))

## 8.2 `io.grpc.netty.NettyServerBuilder`

| Method | Fungsi |
|---|---|
| `NettyServerBuilder.forPort(int)` | Builder Netty server |
| `sslContext(SslContext)` | TLS/mTLS advanced |
| `maxConcurrentCallsPerConnection(int)` | Limit concurrent RPC per connection |
| `flowControlWindow(int)` | HTTP/2 flow control window |
| `bossEventLoopGroup(...)` | Advanced Netty event loop |
| `workerEventLoopGroup(...)` | Advanced Netty worker event loop |
| `channelType(...)` | Advanced Netty channel type |
| `withChildOption(...)` | Advanced Netty socket option |
| `maxInboundMessageSize(int)` | Max message size |
| `maxInboundMetadataSize(int)` | Max metadata size |
| `permitKeepAliveTime(...)` | Keepalive enforcement |
| `permitKeepAliveWithoutCalls(...)` | Keepalive policy |

`NettyServerBuilder.maxInboundMessageSize(int)` mengatur ukuran maksimum message yang boleh diterima server; `maxInboundMetadataSize(int)` mengatur batas metadata, dan connection management methods seperti max connection age/idle/keepalive tersedia untuk deployment production. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/netty/NettyServerBuilder.html))

---

# 9. `io.grpc.Server` Lifecycle Cheat Sheet

| Method | Fungsi |
|---|---|
| `start()` | Bind dan start server |
| `awaitTermination()` | Block sampai terminated |
| `awaitTermination(long, TimeUnit)` | Tunggu terminate dengan timeout |
| `shutdown()` | Orderly shutdown: reject new calls, existing calls boleh lanjut |
| `shutdownNow()` | Forceful shutdown |
| `isShutdown()` | Server sudah shutdown? |
| `isTerminated()` | Semua call/resource sudah selesai? |
| `getPort()` | Port yang didengar |
| `getServices()` | List service terdaftar, experimental |
| `getListenSockets()` | Listening sockets |

`Server.shutdown()` memulai orderly shutdown: new calls ditolak, existing calls masih dapat berjalan; untuk menunggu existing calls selesai, panggil `awaitTermination()`. `shutdownNow()` menolak preexisting dan new calls secara forceful. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/Server.html))
...
public void shutdownGracefully() throws InterruptedException {
    ...
    if (!server.awaitTermination(30, TimeUnit.SECONDS)) {
        ...
        if (!server.awaitTermination(10, TimeUnit.SECONDS)) {
            log.warn("gRPC server did not terminate");
        }
    }
}
```
...
Server server = NettyServerBuilder
        ...
        .intercept(new AuthenticationServerInterceptor(tokenVerifier))
        .intercept(new GrpcServerLoggingInterceptor(logger))
...
# 11. Interceptor Cheat Sheet

## 11.1 Core Classes

| Class / Interface | Fungsi |
|---|---|
| `ServerInterceptor` | Middleware untuk incoming server calls |
| `ClientInterceptor` | Middleware untuk outgoing client calls |
| `ServerCall<ReqT, RespT>` | Abstraksi call server |
| `ServerCall.Listener<ReqT>` | Listener incoming messages |
| `ServerCallHandler<ReqT, RespT>` | Next handler chain |
| `ForwardingServerCall` | Decorator server call |
| `ForwardingServerCallListener` | Decorator listener |
| `ServerInterceptors` | Utility intercept per service |
| `Contexts` | Utility attach `Context` ke call |
| `Context` | gRPC context propagation |
| `Metadata` | Header/trailer gRPC |

Interceptor cocok untuk behavior cross-cutting seperti metadata handling, logging, metrics, policy enforcement, server-side authentication, dan authorization. Interceptor bisa ditambahkan saat membangun server/channel dan dipanggil untuk setiap RPC; interceptor bersifat per-call, bukan untuk konfigurasi TCP port/TLS. ([gRPC](https://grpc.io/docs/guides/interceptors/))

## 11.2 Server Interceptor Template
...
`ServerInterceptor.interceptCall()` menerima `ServerCall`, `Metadata headers`, dan `ServerCallHandler next`; implementasi harus mengembalikan listener non-null, dan headers dapat berisi metadata call seperti authentication credentials. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/ServerInterceptor.html))

---

# 12. Metadata Cheat Sheet
...
| API | Fungsi |
|---|---|
| `Metadata.Key.of(String, Metadata.AsciiMarshaller<T>)` | Key ASCII metadata |
| `Metadata.Key.of(String, Metadata.BinaryMarshaller<T>)` | Key binary metadata |
| `headers.get(key)` | Ambil metadata |
| `headers.put(key, value)` | Set metadata |
| `headers.remove(key, value)` | Remove value |
| `Metadata.ASCII_STRING_MARSHALLER` | Marshaller string |
| `Metadata.BINARY_BYTE_MARSHALLER` | Marshaller byte array |
| Key suffix `-bin` | Wajib untuk binary metadata |

Praktik metadata:

| Metadata | Pakai untuk |
|---|---|
| `authorization` | Bearer token / service token |
| `x-correlation-id` | Log correlation |
| `x-tenant-id` | Tenant context setelah diverifikasi |
| `x-actor-id` | Actor context setelah diverifikasi |
| `traceparent` | Trace context |
| `x-source-service` | Caller service |
| `x-purpose` | Audit/purpose context |

Jangan masukkan payload bisnis besar, PII, password, atau data sensitif yang tidak perlu ke metadata.

---

# 13. Status / Error Handling Cheat Sheet

| Class | Fungsi |
|---|---|
| `Status` | gRPC canonical status |
| `Status.Code` | Enum code: `NOT_FOUND`, `UNAVAILABLE`, dll |
| `StatusRuntimeException` | Runtime exception status |
| `StatusException` | Checked exception status |
| `Status.fromThrowable(Throwable)` | Extract status dari throwable |
| `Status.asRuntimeException()` | Convert status ke runtime exception |
| `Status.asException()` | Convert status ke checked exception |
| `StatusProto` | Convert `google.rpc.Status` ke exception |
| `com.google.rpc.Status` | Rich error model |
| `com.google.rpc.BadRequest` | Field validation detail |
| `com.google.rpc.ErrorInfo` | Structured error reason/domain |
| `com.google.rpc.RetryInfo` | Retry delay detail |
| `com.google.protobuf.Any` | Pack rich error detail |
...
responseObserver.onError(
        Status.FAILED_PRECONDITION
                .withDescription("Case is already closed")
                .asRuntimeException()
);
```
...
# 14. Rich Error Detail Template

```java id="3ndv72"
public RuntimeException invalidArgumentWithFieldViolations(
        List<FieldViolation> violations
) {
    BadRequest.Builder badRequest = BadRequest.newBuilder();
...
Gunakan rich error detail hanya jika client butuh structured detail; jangan untuk stack trace/debug internal.

---

# 15. Context Cheat Sheet

| API | Fungsi |
|---|---|
| `Context.current()` | Ambil current gRPC context |
| `Context.key(String)` | Buat typed key |
| `context.withValue(key, value)` | Tambah value |
| `Contexts.interceptCall(context, call, headers, next)` | Attach context ke call |
| `Context.current().isCancelled()` | Cek cancellation |
| `Context.current().getDeadline()` | Ambil deadline jika ada |
| `Context.CancellableContext` | Context yang bisa dicancel |
...
```java id="1bippk"
String correlationId = RequestContextKeys.CORRELATION_ID.get();
...
# 16. Streaming Server Cheat Sheet

## 16.1 Server Streaming
...
public void streamCaseEvents(
    ...
    try {
        for (CaseEventView event : eventService.findEvents(request.getCaseId())) {
            if (Context.current().isCancelled()) {
                return;
            }
...
public StreamObserver<EvidenceChunk> uploadEvidence(
    ...
    return new StreamObserver<>() {
        @Override
        public void onNext(EvidenceChunk chunk) {
            uploadService.append(chunk);
        }

        @Override
        public void onError(Throwable throwable) {
            uploadService.markFailed(throwable);
        }

        @Override
        public void onCompleted() {
            UploadEvidenceResponse response = uploadService.complete();
            responseObserver.onNext(response);
            responseObserver.onCompleted();
        }
    };
...
## 16.3 Manual Flow Control

| Class | Fungsi |
|---|---|
| `CallStreamObserver<V>` | Base observer dengan flow control |
| `ServerCallStreamObserver<V>` | Server-side stream observer |
| `ClientCallStreamObserver<V>` | Client-side stream observer |
| `isReady()` | Apakah stream siap menerima message lagi |
| `setOnReadyHandler(Runnable)` | Callback saat siap |
| `setOnCancelHandler(Runnable)` | Callback saat client cancel |
| `disableAutoInboundFlowControl()` | Manual inbound flow control, advanced |
| `request(int count)` | Request more messages, advanced |

Contoh server streaming dengan readiness:
...
serverObserver.setOnReadyHandler(() -> {
    while (serverObserver.isReady() && iterator.hasNext()) {
        ...
        serverObserver.onNext(mapper.toProto(iterator.next()));
    }
...
# 17. Health Check Cheat Sheet

| Class | Package | Fungsi |
|---|---|---|
| `HealthStatusManager` | `io.grpc.protobuf.services.HealthStatusManager` | Manage health status service |
| `HealthCheckResponse.ServingStatus` | `io.grpc.health.v1` | `SERVING`, `NOT_SERVING`, `UNKNOWN` |
| `getHealthService()` | method | Register ke server |
| `setStatus(service, status)` | method | Set health status |
| `clearStatus(service)` | method | Clear status |
| `enterTerminalState()` | method | Mark not serving terminal |

`HealthStatusManager` membuat health check service dan dapat mengambilnya via `getHealthService()` serta mengubah status server via `setStatus()`. Untuk package lama `io.grpc.services.HealthStatusManager`, API tersebut deprecated dan diarahkan ke `io.grpc.protobuf.services.HealthStatusManager`. ([GitHub](https://github.com/grpc/grpc-java/blob/master/services/src/main/java/io/grpc/services/HealthStatusManager.java?utm_source=chatgpt.com))
...
# 18. Reflection Cheat Sheet

| Class | Fungsi |
|---|---|
| `ProtoReflectionService.newInstance()` | Register gRPC reflection service |
| `ServerReflectionGrpc` | Generated reflection API |
| `grpcurl` | Tool umum untuk inspect/call gRPC |
...
| Local | Boleh aktif |
| Integration test | Boleh aktif |
| Staging | Boleh dengan akses terbatas |
| Production | Default disable/restrict |

---

# 19. TLS / mTLS Cheat Sheet

## 19.1 Simple TLS dengan `ServerBuilder`

```java id="7cop61"
Server server = ServerBuilder
        .forPort(9090)
        .useTransportSecurity(
                new File("/etc/tls/server.crt"),
                new File("/etc/tls/server.key")
        )
        .addService(caseQueryGrpcService)
        .build();
```

`useTransportSecurity(certChain, privateKey)` membuat server memakai TLS. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/ServerBuilder.html))

## 19.2 Netty mTLS Advanced
...
SslContext sslContext = GrpcSslContexts.forServer(
        ...
        .trustManager(new File("/etc/tls/ca.crt"))
        .clientAuth(ClientAuth.REQUIRE)
        .build();
...
| Class | Fungsi |
|---|---|
| `GrpcSslContexts` | Helper SSL context gRPC |
| `SslContext` | Netty SSL config |
| `ClientAuth.REQUIRE` | Wajib client certificate |
| `NettyServerBuilder.sslContext(...)` | Pasang TLS/mTLS context |

---

# 20. Security Server Interceptor Cheat Sheet
...
public final class AuthenticationServerInterceptor implements ServerInterceptor {
    ...
    public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(
        ...
        AuthenticatedPrincipal principal =
                tokenVerifier.verify(authorization.substring("Bearer ".length()));

        Context context = Context.current()
                .withValue(RequestContextKeys.PRINCIPAL, principal);
...
| Authenticated but no permission | `PERMISSION_DENIED` |
| Tenant mismatch hidden | `NOT_FOUND` atau `PERMISSION_DENIED` sesuai policy |
| Invalid security metadata | `INVALID_ARGUMENT` atau `UNAUTHENTICATED` |

---

# 21. Observability Classes / Hooks

| Class / API | Fungsi |
|---|---|
| `ServerInterceptor` | Logging/metrics/auth/context |
| `ServerStreamTracer.Factory` | Traffic tracing, advanced |
| `MetricSink` | gRPC metric sink |
| `BinaryLog` | Binary logging, advanced |
| `ForwardingServerCall` | Hook `close()` status/duration |
| `ForwardingServerCallListener` | Hook `onMessage`, `onHalfClose`, etc. |
| `Status.Code` | Label status metrics |
| `MethodDescriptor.getFullMethodName()` | Nama method untuk log/metrics |
| `Metadata` | Correlation/trace context |
        ...
        ServerCall<ReqT, RespT> wrapped =
                new ForwardingServerCall.SimpleForwardingServerCall<>(call) {
                    ...
                    public void close(Status status, Metadata trailers) {
                        ...
                        log.info(
                                "grpc_server_call method={} status={} duration_ms={}",
                                method,
                                status.getCode(),
                                durationMs
                        );
...
# 22. Important Server Config Properties

Gunakan external config/env/application properties, bukan hardcode.

```properties id="h28fqr"
grpc.server.port=9090
grpc.server.reflection.enabled=false
grpc.server.health.enabled=true
grpc.server.max-inbound-message-size=8388608
grpc.server.max-inbound-metadata-size=16384
grpc.server.shutdown-grace-seconds=30

grpc.server.executor.threads=64

grpc.server.tls.enabled=true
grpc.server.tls.cert-chain=/etc/tls/server.crt
grpc.server.tls.private-key=/etc/tls/server.key
grpc.server.tls.ca-cert=/etc/tls/ca.crt
grpc.server.mtls.enabled=true

grpc.server.keepalive.permit-time-seconds=300
grpc.server.keepalive.permit-without-calls=false
grpc.server.connection.max-idle-seconds=600
grpc.server.connection.max-age-seconds=1800
grpc.server.connection.max-age-grace-seconds=60

grpc.server.security.auth-required=true
grpc.server.security.required-audience=case-platform
grpc.server.security.allowed-clock-skew-seconds=60

grpc.server.observability.log-payload=false
grpc.server.observability.metrics-enabled=true
grpc.server.observability.tracing-enabled=true
```

| Property | Fungsi |
|---|---|
| `grpc.server.port` | Port gRPC |
| `grpc.server.health.enabled` | Register health service |
| `grpc.server.reflection.enabled` | Reflection dev/test |
| `grpc.server.max-inbound-message-size` | Limit payload |
| `grpc.server.max-inbound-metadata-size` | Limit headers/metadata |
| `grpc.server.shutdown-grace-seconds` | Graceful shutdown |
| `grpc.server.executor.threads` | Worker threads |
| `grpc.server.tls.enabled` | TLS |
| `grpc.server.mtls.enabled` | Client cert required |
| `grpc.server.keepalive.*` | Keepalive policy |
| `grpc.server.connection.*` | Connection lifecycle |
| `grpc.server.security.*` | Auth config |
| `grpc.server.observability.*` | Logging/metrics/tracing |

---

# 23. Server Builder Config Mapping

| Property | Builder Method |
|---|---|
| `grpc.server.port` | `NettyServerBuilder.forPort(port)` |
| `grpc.server.executor.threads` | `.executor(executor)` |
| `grpc.server.max-inbound-message-size` | `.maxInboundMessageSize(bytes)` |
| `grpc.server.max-inbound-metadata-size` | `.maxInboundMetadataSize(bytes)` |
| `grpc.server.tls.cert/private-key` | `.useTransportSecurity(cert, key)` or `.sslContext(...)` |
| `grpc.server.keepalive.time` | `.keepAliveTime(value, unit)` |
| `grpc.server.keepalive.timeout` | `.keepAliveTimeout(value, unit)` |
| `grpc.server.keepalive.permit-time` | `.permitKeepAliveTime(value, unit)` |
| `grpc.server.keepalive.permit-without-calls` | `.permitKeepAliveWithoutCalls(boolean)` |
| `grpc.server.connection.max-idle` | `.maxConnectionIdle(value, unit)` |
| `grpc.server.connection.max-age` | `.maxConnectionAge(value, unit)` |
| `grpc.server.connection.max-age-grace` | `.maxConnectionAgeGrace(value, unit)` |
| Global interceptors | `.intercept(interceptor)` |
| Per-service interceptors | `ServerInterceptors.intercept(service, interceptors...)` |
| Health service | `.addService(health.getHealthService())` |
| Reflection | `.addService(ProtoReflectionService.newInstance())` |

---

# 24. Important Packages

| Package | Isi |
|---|---|
| `io.grpc` | Core API: `Server`, `ServerBuilder`, `Status`, `Metadata`, interceptor |
| `io.grpc.stub` | `StreamObserver`, stubs, streaming helpers |
| `io.grpc.netty` | Netty transport builder |
| `io.grpc.netty.shaded` | Shaded Netty transport |
| `io.grpc.protobuf` | Protobuf integration, `StatusProto` |
| `io.grpc.protobuf.services` | Health/reflection/services |
| `io.grpc.health.v1` | Health proto classes |
| `io.grpc.stub.annotations` | `@GrpcGenerated` |
| `com.google.rpc` | Rich error model |
| `com.google.protobuf` | Protobuf message/runtime |

---

# 25. Testing Server Cheat Sheet

| Class / Tool | Fungsi |
|---|---|
| `InProcessServerBuilder` | Server test tanpa port |
| `InProcessChannelBuilder` | Channel test tanpa network |
| `GrpcCleanupRule` | Cleanup JUnit rule, jika pakai JUnit4 |
| `StreamRecorder` | Testing observer, deprecated/not public use di beberapa versi; hati-hati |
| Fake `StreamObserver` | Test streaming manual |
| Testcontainers PostgreSQL | Integration DB |
| `grpcurl` | Manual call/test |
| `grpc-health-probe` | Health check CLI |

Contoh in-process test:
...
| Area | Checklist |
|---|---|
| Proto | `java_package`, `java_multiple_files`, versioning benar |
| Server impl | Extend `ImplBase`, handler tipis |
| Builder | Port, executor, limits, TLS, interceptors configured |
| Lifecycle | `start`, `shutdown`, `awaitTermination`, `shutdownNow` benar |
| Health | Health service registered, status berubah saat drain |
| Reflection | Disabled/restricted production |
| Security | Auth/mTLS/token/interceptor/tenant context |
| Error | `Status` mapping spesifik, no internal leak |
| Streaming | Flow control, cancellation, max size/duration |
| Observability | logs/metrics/traces/correlation ID |
| Threading | Jangan `directExecutor` untuk blocking MyBatis/JDBC |
| Backpressure | Executor/DB pool/bulkhead sesuai |
| Kubernetes | readiness/liveness/graceful shutdown |
| Config | Semua env-specific externalized |
| Tests | In-process, metadata, error, timeout, security |
...
| Anti-Pattern | Dampak |
|---|---|
| Handler langsung akses MyBatis | Boundary kacau |
| Semua exception jadi `INTERNAL` | Client tidak bisa decision |
| Tidak ada `maxInboundMessageSize` | Payload abuse/OOM |
| Reflection aktif production terbuka | API surface exposure |
| Tidak ada health service | Kubernetes/readiness buruk |
| `directExecutor()` untuk blocking DB | Transport thread bisa blocked |
| Tidak shutdown custom executor | Thread leak |
| Channel/client dibuat di server handler untuk self-call | Overhead/coupling |
| Metadata auth dipercaya tanpa verifikasi | Spoofing |
| Log full proto payload | PII/security risk |
| Stream tanpa cancellation handling | Resource leak |
| Tidak set readiness `NOT_SERVING` saat shutdown | `UNAVAILABLE` spike saat rollout |

---

# 28. Mini Template Production-Style

```java id="2kyexf"
public final class GrpcServerModule implements AutoCloseable {
    ...
    public GrpcServerModule(
            ...
            CaseQueryGrpcService caseQueryService,
            CaseCommandGrpcService caseCommandService,
        ...
        NettyServerBuilder builder = NettyServerBuilder
                .forPort(config.port())
                .executor(executor)
                .addService(caseQueryService)
                .addService(caseCommandService)
                .addService(health.getHealthService())
                .intercept(new CorrelationIdServerInterceptor())
                .intercept(new AuthenticationServerInterceptor(tokenVerifier))
                .intercept(new GrpcServerLoggingInterceptor())
...
# 29. Ringkasan Super Cepat

```text id="cua2cj"
Java gRPC Server essentials:

- Contract ada di .proto
- Server implementation extend ServiceNameGrpc.ServiceNameImplBase
- Method unary/server-streaming pakai StreamObserver<Response>
- Client/bidi streaming return StreamObserver<Request>
- Register service via ServerBuilder.addService(...)
- Cross-cutting concern via ServerInterceptor
- Metadata untuk auth/correlation/tenant/trace
- Context untuk propagation internal per call
- Error via Status / StatusRuntimeException
- Streaming harus handle flow control dan cancellation
- Health via HealthStatusManager
- Reflection hanya dev/test atau restricted
- TLS via useTransportSecurity atau Netty sslContext
- Shutdown via shutdown + awaitTermination + shutdownNow
- Jangan directExecutor untuk blocking MyBatis/JDBC
- Jangan log payload/token/PII
```

Rekomendasi materi berikutnya: **Cheat Sheet Java gRPC Client**-`ManagedChannel`, stub types, deadlines, retry/service config, metadata/call credentials, TLS/mTLS, load balancing, dan failure handling.
...
- [Generated-code reference | Java | gRPC](https://grpc.io/docs/languages/java/generated-code/)
- [GrpcGenerated (grpc-all 1.82.0 API)](https://grpc.github.io/grpc-java/javadoc/io/grpc/stub/annotations/GrpcGenerated.html?utm_source=chatgpt.com)
- [HealthStatusManager.java](https://github.com/grpc/grpc-java/blob/master/services/src/main/java/io/grpc/services/HealthStatusManager.java?utm_source=chatgpt.com)

## Response:
7/15/2026, 1:48:12 PM

Saya akan susun cheat sheet khusus gRPC Java client: mulai dari generated stub, `ManagedChannel`, deadline, metadata/interceptor, retry/service config, TLS/mTLS, load balancing, sampai property/config yang biasa dipakai di production.

Berikut **cheat sheet Java gRPC Client** untuk konteks Java 17+, Maven, Jersey/JAX-RS microservice, Kubernetes, on-prem/cloud.

> Catatan utama: gRPC Java client **tidak annotation-heavy** seperti JAX-RS. Mayoritas penggunaan client dilakukan melalui **generated stub**, `ManagedChannel`, `ManagedChannelBuilder`/`NettyChannelBuilder`, `AbstractStub` options, `ClientInterceptor`, `CallCredentials`, `Metadata`, deadline, service config, dan lifecycle channel.
...
<dependencies>
    <!-- Core API: Channel, ManagedChannel, Metadata, Status, Interceptor -->
    <dependency>
        <groupId>io.grpc</groupId>
        <artifactId>grpc-api</artifactId>
    </dependency>

    <!-- Generated stubs + StreamObserver -->
    <dependency>
        <groupId>io.grpc</groupId>
        <artifactId>grpc-stub</artifactId>
    </dependency>
    ...
    <!-- Netty transport, commonly used for production client -->
    <dependency>
        <groupId>io.grpc</groupId>
        <artifactId>grpc-netty-shaded</artifactId>
    </dependency>
    ...
    <!-- Rich error detail: google.rpc.Status, BadRequest, RetryInfo, ErrorInfo -->
    <dependency>
        <groupId>com.google.api.grpc</groupId>
        <artifactId>proto-google-common-protos</artifactId>
        <version>${google.common.protos.version}</version>
...
# 2. Proto Options yang Mempengaruhi Client
...
| Option / Element | Fungsi untuk Client |
|---|---|
| `service` | Menghasilkan `ServiceNameGrpc` dan stub classes |
| `rpc` | Menghasilkan method Java di stub |
| `message` | Menghasilkan request/response Java class |
| `java_package` | Package generated Java client class |
| `java_multiple_files = true` | Message/enum/service generated lebih nyaman dipakai |
| `java_outer_classname` | Nama wrapper class stabil |
| `package` | Namespace protobuf/gRPC method full name |

Generated Java client class menyediakan stub yang membungkus `Channel`; gRPC Java menghasilkan tiga jenis stub utama: async, blocking, dan future. ([gRPC](https://grpc.io/docs/languages/java/generated-code/))

---

# 3. Annotation Cheat Sheet

## 3.1 Annotation gRPC / Generated

| Annotation | Package | Dipakai di Mana | Catatan |
|---|---|---|---|
| `@GrpcGenerated` | `io.grpc.stub.annotations.GrpcGenerated` | Generated code | Jangan ditulis manual di application code |
| `@ExperimentalApi` | `io.grpc.ExperimentalApi` | API gRPC tertentu | API bisa berubah; hati-hati untuk production library |
| `@Internal` | `io.grpc.Internal` | API internal gRPC | Hindari di application code |
| `@Generated` | `javax.annotation.Generated` atau variasi | Generated source | Kadang butuh `javax.annotation-api` |
| `@Deprecated` | Java standard | API lama/deprecated | Perhatikan saat membaca contoh lama |

## 3.2 Annotation Non-gRPC yang Sering Dipakai untuk Client Wrapper

| Annotation | Sumber | Fungsi |
|---|---|---|
| `@ApplicationScoped` | CDI/Weld | Singleton client bean |
| `@Singleton` | DI framework | Singleton client/channel factory |
| `@Inject` | CDI/Jakarta | Constructor injection |
| `@Named` | CDI/Jakarta | Qualifier |
| `@PostConstruct` | Jakarta annotation | Build channel saat startup |
| `@PreDestroy` | Jakarta annotation | Shutdown channel saat stop |
| `@Provider` | JAX-RS | REST exception mapper, bukan gRPC client |
| `@Slf4j` | Lombok | Logging helper |

gRPC client call sendiri tidak memakai annotation seperti `@GET`, `@POST`, atau `@Path`. Itu tetap milik JAX-RS boundary.

---

# 4. Generated Stub Cheat Sheet
...
Generated Java biasanya menyediakan:

| Generated API | Fungsi |
|---|---|
| `CaseQueryServiceGrpc` | Outer gRPC generated class |
| `CaseQueryServiceGrpc.newStub(Channel)` | Async stub |
| `CaseQueryServiceGrpc.newBlockingStub(Channel)` | Blocking stub |
| `CaseQueryServiceGrpc.newFutureStub(Channel)` | Future stub |
| `CaseQueryServiceStub` | Async/callback client |
| `CaseQueryServiceBlockingStub` | Blocking/synchronous client |
| `CaseQueryServiceFutureStub` | Future unary client |
| `getServiceDescriptor()` | Service descriptor |
| `getGetCaseMethod()` | Method descriptor |
| `MethodDescriptor` | Metadata method-level internal gRPC |

Generated async stub menggunakan `StreamObserver`, blocking stub hanya mendukung unary dan server-streaming, sedangkan future stub hanya mendukung unary call. ([gRPC](https://grpc.io/docs/languages/java/generated-code/))

---

# 5. Stub Type Cheat Sheet

| Stub | Generated Class | Cocok untuk | Tidak Cocok untuk |
|---|---|---|---|
| Async stub | `ServiceNameStub` | Streaming, async unary, callback | Code sederhana yang blocking |
| Blocking stub | `ServiceNameBlockingStub` | Jersey/JAX-RS blocking flow, simple query | Client-streaming / bidi-streaming |
| Future stub | `ServiceNameFutureStub` | Parallel unary call, future composition | Streaming |
...
# 6. RPC Method Signature di Client

| RPC Type | Proto | Blocking Stub | Async Stub | Future Stub |
|---|---|---|---|---|
| Unary | `rpc GetCase(A) returns (B)` | `B getCase(A request)` | `void getCase(A request, StreamObserver<B>)` | `ListenableFuture<B> getCase(A request)` |
| Server streaming | `rpc Stream(A) returns (stream B)` | `Iterator<B> stream(A request)` | `void stream(A request, StreamObserver<B>)` | Tidak |
| Client streaming | `rpc Upload(stream A) returns (B)` | Tidak | `StreamObserver<A> upload(StreamObserver<B>)` | Tidak |
| Bidi streaming | `rpc Chat(stream A) returns (stream B)` | Tidak | `StreamObserver<A> chat(StreamObserver<B>)` | Tidak |

---

# 7. `ManagedChannel` Cheat Sheet

`ManagedChannel` adalah resource long-lived untuk komunikasi ke target service. Jangan dibuat per request.

| Method | Fungsi |
|---|---|
| `shutdown()` | Orderly shutdown; call yang sudah berjalan boleh lanjut, call baru langsung dibatalkan |
| `shutdownNow()` | Forceful shutdown; preexisting dan new calls dibatalkan |
| `awaitTermination(timeout, unit)` | Tunggu channel selesai terminate |
| `isShutdown()` | Channel sudah shutdown? |
| `isTerminated()` | Semua call/resource sudah selesai? |
| `getState(boolean requestConnection)` | Ambil connectivity state |
| `notifyWhenStateChanged(...)` | Callback saat state berubah |
| `newCall(...)` | Low-level call creation, biasanya tidak dipakai langsung |

`ManagedChannel.shutdown()` melakukan orderly shutdown, sedangkan `shutdownNow()` melakukan forceful shutdown; terminated berarti tidak ada running calls dan resource seperti TCP connection sudah dilepas. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/ManagedChannel.html))
...
# 8. `ManagedChannelBuilder` Cheat Sheet

| Method | Fungsi |
|---|---|
| `forAddress(String host, int port)` | Buat channel ke host/port |
| `forTarget(String target)` | Buat channel ke target URI/name resolver |
| `usePlaintext()` | Non-TLS, local/dev/internal tertentu |
| `useTransportSecurity()` | TLS default |
| `executor(Executor)` | Custom executor untuk callback/application logic |
| `directExecutor()` | Jalankan app code di transport thread; hindari blocking |
| `intercept(ClientInterceptor...)` | Pasang client interceptors |
| `userAgent(String)` | Set user-agent |
| `overrideAuthority(String)` | Override authority/SNI/testing |
| `defaultLoadBalancingPolicy(String)` | Default LB policy jika service config tidak menentukan |
| `defaultServiceConfig(Map<String, ?>)` | Set service config manual |
| `disableServiceConfigLookUp()` | Disable service config dari resolver |
| `enableRetry()` | Enable retry/hedging subsystem |
| `disableRetry()` | Disable retry/hedging subsystem |
| `maxRetryAttempts(int)` | Limit retry attempts dari service config |
| `maxHedgedAttempts(int)` | Limit hedging attempts |
| `retryBufferSize(long)` | Buffer retry global |
| `perRpcBufferLimit(long)` | Buffer retry per RPC |
| `idleTimeout(long, TimeUnit)` | Channel idle timeout |
| `keepAliveTime(long, TimeUnit)` | Keepalive ping interval |
| `keepAliveTimeout(long, TimeUnit)` | Timeout waiting keepalive ack |
| `keepAliveWithoutCalls(boolean)` | Ping walau tidak ada RPC aktif |
| `maxInboundMessageSize(int)` | Limit inbound response message |
| `maxInboundMetadataSize(int)` | Limit inbound metadata |
| `proxyDetector(ProxyDetector)` | Proxy resolution |
| `offloadExecutor(Executor)` | Executor untuk blocking/offloaded operations |
| `build()` | Build `ManagedChannel` |

`ManagedChannelBuilder` menyediakan konfigurasi seperti `forAddress`, `executor`, `directExecutor`, `enableRetry`, `disableRetry`, `defaultServiceConfig`, dan `defaultLoadBalancingPolicy`; retry yang diaktifkan akan memakai per-method configuration, dan tanpa konfigurasi method, retry dibatasi ke transparent retries. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/ManagedChannelBuilder.html))

---

# 9. `NettyChannelBuilder` Cheat Sheet

Gunakan `NettyChannelBuilder` saat butuh control TLS/mTLS, Netty tuning, flow control, atau transport-level config.

| Method | Fungsi |
|---|---|
| `forAddress(host, port)` | Buat Netty channel |
| `forTarget(target)` | Buat Netty channel via resolver |
| `usePlaintext()` | Non-TLS |
| `sslContext(SslContext)` | TLS/mTLS custom |
| `negotiationType(NegotiationType)` | TLS/plaintext negotiation |
| `flowControlWindow(int)` | HTTP/2 flow control |
| `initialFlowControlWindow(int)` | Initial flow control |
| `maxInboundMessageSize(int)` | Max response size |
| `maxInboundMetadataSize(int)` | Metadata size advisory/limit |
| `keepAliveTime(...)` | HTTP/2 PING |
| `keepAliveTimeout(...)` | Keepalive timeout |
| `keepAliveWithoutCalls(boolean)` | Ping without active call |
| `eventLoopGroup(EventLoopGroup)` | Custom Netty event loop |
| `channelType(Class)` | Netty channel type |
| `withOption(...)` | Socket/channel option |
| `build()` | Build channel |

`NettyChannelBuilder` adalah builder untuk Netty transport; `sslContext(SslContext)` mengatur SSL/TLS context dan harus dikonfigurasi dengan `GrpcSslContexts`, sementara custom `EventLoopGroup` tidak dimiliki channel sehingga caller bertanggung jawab shutdown sendiri. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/netty/NettyChannelBuilder.html))

---

# 10. `AbstractStub` Important Methods

Semua generated stub extend keluarga `AbstractStub`.

| Method | Fungsi |
|---|---|
| `withDeadlineAfter(long, TimeUnit)` | Set deadline relatif |
| `withDeadline(Deadline)` | Set absolute deadline |
| `withWaitForReady()` | Queue RPC sampai connection available |
| `withCallCredentials(CallCredentials)` | Attach per-RPC credentials |
| `withInterceptors(ClientInterceptor...)` | Attach interceptors ke stub |
| `withCompression(String)` | Compress outbound messages, misalnya `"gzip"` |
| `withExecutor(Executor)` | Executor untuk callback stub |
| `withMaxInboundMessageSize(int)` | Limit response size |
| `withMaxOutboundMessageSize(int)` | Limit request size |
| `withOption(CallOptions.Key<T>, T)` | Custom option untuk interceptor |
| `withChannel(Channel)` | Deprecated; prefer interceptors |

`AbstractStub.withDeadlineAfter(...)` membuat stub baru dengan deadline relatif, `withCallCredentials(...)` membuat stub baru dengan call credentials, dan `withWaitForReady()` membuat call menunggu koneksi tersedia, tetapi dapat meningkatkan latency. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/stub/AbstractStub.html))
...
# 11. Deadline Cheat Sheet

Deadline wajib untuk production gRPC client.
...
| Praktik | Penjelasan |
|---|---|
| Set deadline per call/use case | Query cepat dan command workflow berbeda |
| Deadline < upstream HTTP timeout | Sisakan waktu error mapping |
| Jangan retry melewati deadline total | Hindari request chain menggantung |
| Log deadline dan status | Debug `DEADLINE_EXCEEDED` |
| Propagate remaining deadline | Jangan reset timeout besar di tiap hop |

gRPC deadline mendefinisikan titik waktu ketika client tidak lagi mau menunggu response; secara default gRPC tidak menetapkan deadline, sehingga client bisa menunggu sangat lama jika tidak eksplisit diberi deadline. ([gRPC](https://grpc.io/docs/guides/deadlines/))

---

# 12. Basic Client Wrapper Template
...
public final class CaseQueryGrpcClient {
    ...
    private CaseView map(GetCaseResponse response) {
        return new CaseView(
                response.getCaseId(),
                response.getCaseNumber(),
...
```text id="nmgl83"
Application service jangan langsung expose generated stub.
Bungkus stub dalam client adapter/wrapper.
```

---

# 13. `Metadata` Cheat Sheet

| API | Fungsi |
|---|---|
| `Metadata.Key.of(name, ASCII_STRING_MARSHALLER)` | String metadata key |
| `Metadata.Key.of(name, BINARY_BYTE_MARSHALLER)` | Binary metadata key |
| `metadata.put(key, value)` | Tambah value |
| `metadata.get(key)` | Ambil value |
| `metadata.remove(key, value)` | Remove |
| `MetadataUtils.attachHeaders(stub, metadata)` | Attach static headers ke stub |
| `ClientInterceptor` | Dynamic metadata per call |
| `CallCredentials` | Auth credentials per RPC |

gRPC metadata dapat membawa authentication credentials seperti OAuth2/JWT via Authorization header dan tracing information; gunakan metadata untuk request context, bukan payload bisnis besar. ([gRPC](https://grpc.io/docs/guides/metadata/?utm_source=chatgpt.com))
...
# 14. `ClientInterceptor` Cheat Sheet

| Interface / Class | Fungsi |
|---|---|
| `ClientInterceptor` | Intercept outgoing calls |
| `interceptCall(...)` | Hook call creation |
| `ClientCall` | Low-level outgoing call |
| `ForwardingClientCall` | Decorate call |
| `ForwardingClientCallListener` | Decorate response listener |
| `ClientInterceptors.intercept(...)` | Wrap channel dengan interceptor |

`ClientInterceptor` dipakai untuk cross-cutting behavior sebelum outgoing call dikirim, misalnya logging, monitoring, metadata, atau request/response rewriting; untuk authentication credentials, javadoc menyarankan `CallCredentials` lebih tepat. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/ClientInterceptor.html))

Contoh metadata interceptor:
    ...
    public <ReqT, RespT> ClientCall<ReqT, RespT> interceptCall(
        ...
        return new ForwardingClientCall.SimpleForwardingClientCall<>(delegate) {
            ...
            public void start(Listener<RespT> responseListener, Metadata headers) {
                ...
                putIfPresent(headers, GrpcMetadataKeys.CORRELATION_ID, context.correlationId());
                putIfPresent(headers, GrpcMetadataKeys.TENANT_ID, context.tenantId());
...
# 15. `CallCredentials` Cheat Sheet

`CallCredentials` cocok untuk per-RPC credentials seperti bearer token, terutama jika token bisa berubah per request.

| Class / Method | Fungsi |
|---|---|
| `CallCredentials` | Credential data propagated via request metadata |
| `applyRequestMetadata(...)` | Generate metadata per RPC |
| `MetadataApplier.apply(metadata)` | Apply generated metadata |
| `MetadataApplier.fail(status)` | Fail call karena credentials error |
| `CompositeCallCredentials` | Gabungkan credentials |
| `stub.withCallCredentials(credentials)` | Pasang credentials ke stub |
| `CallOptions.withCallCredentials(...)` | Pasang credentials via call options |

`CallCredentials` membawa credential data yang dipropagasi ke server via request metadata untuk tiap RPC; `applyRequestMetadata` dipanggil per RPC dan implementasi tidak boleh blocking-jika metadata perlu fetch network, gunakan executor dan panggil applier secara async. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/CallCredentials.html))
...
```java id="xob4hw"
CaseQueryServiceGrpc.CaseQueryServiceBlockingStub securedStub =
        baseStub.withCallCredentials(
                new BearerTokenCallCredentials(tokenProvider)
        );
```

---

# 16. Status / Exception Cheat Sheet

| Class | Fungsi |
|---|---|
| `Status` | Canonical status gRPC |
| `Status.Code` | Enum status |
| `StatusRuntimeException` | Runtime exception dari failed RPC |
| `StatusException` | Checked exception |
| `Status.fromThrowable(Throwable)` | Extract status |
| `Metadata trailers` | Bisa berisi error detail |
| `StatusProto.fromThrowable(Throwable)` | Extract rich error detail |
| `com.google.rpc.Status` | Rich error model |
| `BadRequest`, `RetryInfo`, `ErrorInfo` | Structured detail |

Mapping client:

| gRPC Status | Client Meaning | Retry? |
|---|---|---|
| `INVALID_ARGUMENT` | Request salah | Tidak |
| `NOT_FOUND` | Resource tidak ada/tidak visible | Biasanya tidak |
| `UNAUTHENTICATED` | Token invalid/missing | Tidak, kecuali refresh |
| `PERMISSION_DENIED` | Tidak punya akses | Tidak |
| `FAILED_PRECONDITION` | State tidak valid | Tidak |
| `ABORTED` | Conflict/concurrency | Bisa terbatas |
| `UNAVAILABLE` | Service/dependency unavailable | Ya untuk idempotent |
| `DEADLINE_EXCEEDED` | Timeout | Hati-hati |
| `RESOURCE_EXHAUSTED` | Quota/overload | Backoff |
| `INTERNAL` | Bug/unexpected | Biasanya tidak |
| `UNIMPLEMENTED` | Contract/deployment mismatch | Tidak |
...
public RuntimeException map(String operation, StatusRuntimeException ex) {
    ...
    return switch (code) {
        case NOT_FOUND -> new DownstreamNotFoundException(operation, ex);
        case UNAVAILABLE -> new DownstreamUnavailableException(operation, ex);
        case DEADLINE_EXCEEDED -> new DownstreamTimeoutException(operation, ex);
        case PERMISSION_DENIED -> new DownstreamPermissionDeniedException(operation, ex);
        case UNAUTHENTICATED -> new DownstreamUnauthenticatedException(operation, ex);
        case ABORTED -> new DownstreamConflictException(operation, ex);
        default -> new DownstreamGrpcException(operation, code.name(), ex);
    };
}
```
...
# 17. Retry / Service Config Cheat Sheet

gRPC retry dapat dikontrol melalui service config. Retry memang enabled by default, tetapi tanpa retry policy, gRPC hanya dapat melakukan transparent retry dalam kondisi sangat terbatas ketika yakin RPC belum diproses server. ([gRPC](https://grpc.io/docs/guides/retry/))

Contoh service config:
...
Map<String, Object> methodConfig = Map.of(
        "name", List.of(Map.of(
                "service", "casecommunication.v1.CaseQueryService",
                "method", "GetCase"
        )),
        "timeout", "0.700s",
...
| Service Config Field | Fungsi |
|---|---|
| `methodConfig` | Config per service/method |
| `name.service` | Nama service proto |
| `name.method` | Nama method |
| `timeout` | Timeout per method |
| `retryPolicy.maxAttempts` | Total attempts |
| `initialBackoff` | Backoff awal |
| `maxBackoff` | Backoff maksimum |
| `backoffMultiplier` | Multiplier |
| `retryableStatusCodes` | Status yang boleh retry |
| `hedgingPolicy` | Hedging parallel attempt; hati-hati |
| `healthCheckConfig.serviceName` | Client health checking |

Service config dapat membatasi call behavior ke service/method tertentu dan juga dapat mengaktifkan health checking client lewat `healthCheckConfig`. ([gRPC](https://grpc.io/docs/guides/service-config/))

---

# 18. Retry Rule Practical

| Operation | Retry Policy |
|---|---|
| Query idempotent | Retry `UNAVAILABLE`, mungkin `RESOURCE_EXHAUSTED`, `ABORTED` terbatas |
| Search/list | Retry jika pagination aman |
| Command mutasi | Retry hanya jika punya idempotency key |
| Workflow task completion | Sangat hati-hati |
| Upload/streaming | Perlu resume token/checksum |
| Permission/auth failure | Jangan retry |
| Invalid request | Jangan retry |
...
| Cocok | Tidak Cocok |
|---|---|
| Startup dependency sebentar | User-facing low-latency endpoint |
| Batch job internal | High-QPS endpoint saat outage |
| Rolling deployment singkat | Downstream benar-benar down |
| Non-critical waiting acceptable | Fail-fast requirement |

`withWaitForReady()` membuat stub memakai wait-for-ready; RPC dapat di-queue sampai connection available dan ini dapat meningkatkan latency. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/stub/AbstractStub.html))

---

# 20. Keepalive Cheat Sheet
...
| Setting | Fungsi |
|---|---|
| `keepAliveTime` | Interval HTTP/2 PING |
| `keepAliveTimeout` | Timeout menunggu ack |
| `keepAliveWithoutCalls` | Ping walau tidak ada active RPC |
| Server permit keepalive | Harus disepakati dengan server owner |
| LB/proxy idle timeout | Harus diselaraskan |

HTTP/2 PING-based keepalive menjaga koneksi tetap hidup saat tidak ada data, tetapi intervalnya harus hati-hati; gRPC docs memperingatkan untuk menghindari keepalive tanpa calls dan menghindari konfigurasi terlalu agresif, terutama di bawah sekitar satu menit. ([gRPC](https://grpc.io/docs/guides/keepalive/))

---

# 21. TLS / mTLS Client Cheat Sheet

## 21.1 Plaintext - local/dev only
...
## 21.2 TLS dengan default trust

```java id="ow4tqm"
ManagedChannel channel = NettyChannelBuilder
        .forAddress("case-service.example.com", 9090)
        .useTransportSecurity()
        .build();
```

## 21.3 TLS dengan custom CA
...
## 21.4 mTLS client cert

```java id="156w7z"
SslContext sslContext = GrpcSslContexts.forClient()
        .trustManager(new File("/etc/tls/ca.crt"))
        .keyManager(
                new File("/etc/tls/client.crt"),
                new File("/etc/tls/client.key")
        )
        .build();
...
`NettyChannelBuilder.sslContext(...)` memakai SSL/TLS context, dan javadoc menyebut context itu harus dikonfigurasi dengan `GrpcSslContexts`. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/netty/NettyChannelBuilder.html))

---

# 22. Load Balancing Cheat Sheet

| Config / Class | Fungsi |
|---|---|
| `defaultLoadBalancingPolicy("pick_first")` | Default, pilih satu subchannel |
| `defaultLoadBalancingPolicy("round_robin")` | Distribusi antar resolved addresses |
| Name resolver | Resolve target ke address list |
| Service config | Bisa menentukan LB policy |
| Headless service | Kubernetes endpoint list |
| Service mesh / Envoy | Proxy-level LB |
| Channel pool | Workaround untuk high load tertentu |
...
`defaultLoadBalancingPolicy` menentukan LB policy jika service config tidak menentukan; jika tidak diset, default-nya adalah `pick_first`. ([grpc.github.io](https://grpc.github.io/grpc-java/javadoc/io/grpc/ManagedChannelBuilder.html?utm_source=chatgpt.com))

---

# 23. Health Checking Client

Client-side health checking dapat dikonfigurasi melalui service config agar client hanya mengirim request ke backend yang melaporkan healthy. gRPC health docs menjelaskan bahwa client dapat otomatis berkomunikasi dengan health service backend, dan jika healthy service menjadi unhealthy, client tidak lagi mengirim request untuk service tersebut. ([gRPC](https://grpc.io/docs/guides/health-checking/))

Contoh config:

```java id="788h0q"
Map<String, Object> healthCheckConfig = Map.of(
        "serviceName", "casecommunication.v1.CaseQueryService"
);
...
Catatan: server harus expose standard gRPC health service.

---

# 24. Streaming Client Cheat Sheet

## 24.1 Server Streaming dengan Blocking Stub
...
## 24.2 Server Streaming dengan Async Stub

```java id="iawccf"
asyncStub
        .withDeadlineAfter(5, TimeUnit.SECONDS)
        .streamCaseEvents(request, new StreamObserver<>() {
            @Override
            public void onNext(CaseEvent event) {
                handle(event);
            }
...
## 24.3 Client Streaming

```java id="aywvz4"
StreamObserver<UploadEvidenceResponse> responseObserver =
        new StreamObserver<>() {
            @Override
            public void onNext(UploadEvidenceResponse response) {
...
StreamObserver<EvidenceChunk> requestObserver =
        asyncStub.uploadEvidence(responseObserver);
...
## 24.4 Flow Control Classes

| Class | Fungsi |
|---|---|
| `StreamObserver<T>` | Callback umum |
| `CallStreamObserver<T>` | Observer dengan flow control |
| `ClientCallStreamObserver<T>` | Client-side flow control |
| `ClientResponseObserver<ReqT, RespT>` | Bisa akses request stream sebelum start |
| `isReady()` | Stream siap menerima message |
| `setOnReadyHandler(Runnable)` | Callback saat ready |
| `disableAutoInboundFlowControl()` | Manual inbound flow control |
| `request(int)` | Request more messages |

---

# 25. Client Configuration Properties

```properties id="ol50tn"
grpc.client.case-service.host=case-service
grpc.client.case-service.port=9090
grpc.client.case-service.target=dns:///case-service.default.svc.cluster.local:9090

grpc.client.case-service.plaintext=false
grpc.client.case-service.tls.enabled=true
grpc.client.case-service.tls.ca-cert=/etc/tls/ca.crt
grpc.client.case-service.mtls.enabled=true
grpc.client.case-service.mtls.client-cert=/etc/tls/client.crt
grpc.client.case-service.mtls.client-key=/etc/tls/client.key

grpc.client.case-service.deadline.default-ms=700
grpc.client.case-service.deadline.get-case-ms=500
grpc.client.case-service.deadline.submit-decision-ms=1500

grpc.client.case-service.retry.enabled=true
grpc.client.case-service.retry.max-attempts=3
grpc.client.case-service.retry.initial-backoff-ms=50
grpc.client.case-service.retry.max-backoff-ms=500
grpc.client.case-service.retry.backoff-multiplier=2.0

grpc.client.case-service.load-balancing-policy=round_robin
grpc.client.case-service.health-check.enabled=true
grpc.client.case-service.health-check.service-name=casecommunication.v1.CaseQueryService

grpc.client.case-service.keepalive.enabled=false
grpc.client.case-service.keepalive.time-seconds=60
grpc.client.case-service.keepalive.timeout-seconds=20
grpc.client.case-service.keepalive.without-calls=false

grpc.client.case-service.max-inbound-message-size=8388608
grpc.client.case-service.max-outbound-message-size=8388608
grpc.client.case-service.max-inbound-metadata-size=16384

grpc.client.case-service.compression=gzip
grpc.client.case-service.wait-for-ready=false

grpc.client.case-service.observability.metrics-enabled=true
grpc.client.case-service.observability.tracing-enabled=true
grpc.client.case-service.observability.log-payload=false
```

---

# 26. Config → API Mapping

| Property | Java API |
|---|---|
| `host`, `port` | `ManagedChannelBuilder.forAddress(host, port)` |
| `target` | `ManagedChannelBuilder.forTarget(target)` |
| `plaintext` | `.usePlaintext()` |
| `tls.enabled` | `.useTransportSecurity()` |
| `tls.ca-cert` | `GrpcSslContexts.forClient().trustManager(...)` |
| `mtls.client-cert/key` | `.keyManager(cert, key)` |
| `deadline.*` | `stub.withDeadlineAfter(...)` |
| `retry.enabled` | `channelBuilder.enableRetry()` / `disableRetry()` |
| `retry.*` | `defaultServiceConfig(...)` |
| `load-balancing-policy` | `defaultLoadBalancingPolicy(...)` |
| `health-check.*` | `defaultServiceConfig(healthCheckConfig)` |
| `keepalive.*` | `keepAliveTime`, `keepAliveTimeout`, `keepAliveWithoutCalls` |
| `max-inbound-message-size` | `channelBuilder.maxInboundMessageSize(...)` atau `stub.withMaxInboundMessageSize(...)` |
| `max-outbound-message-size` | `stub.withMaxOutboundMessageSize(...)` |
| `compression` | `stub.withCompression("gzip")` |
| `wait-for-ready` | `stub.withWaitForReady()` |
| `metadata/auth` | `ClientInterceptor` / `CallCredentials` |

---

# 27. Production Client Factory Template

```java id="0f6yex"
public final class CaseGrpcClientFactory implements AutoCloseable {

    private final ManagedChannel channel;
    private final CaseQueryGrpcClient caseQueryClient;

    public CaseGrpcClientFactory(
            GrpcClientConfig config,
            RequestContextProvider requestContextProvider,
        ...
        if (config.target() != null && !config.target().isBlank()) {
            builder = ManagedChannelBuilder.forTarget(config.target());
        } else {
            builder = ManagedChannelBuilder.forAddress(config.host(), config.port());
        }
        ...
        builder
                ...
                .maxInboundMessageSize(config.maxInboundMessageSize())
                .intercept(new ContextPropagationClientInterceptor(requestContextProvider))
                .intercept(new GrpcClientLoggingInterceptor());
        ...
        this.caseQueryClient = new CaseQueryGrpcClient(
                baseStub,
                Duration.ofMillis(config.defaultDeadlineMs()),
                new GrpcClientExceptionMapper()
        );
    }

    public CaseQueryGrpcClient caseQueryClient() {
        return caseQueryClient;
    }
...
# 28. Jersey/JAX-RS Integration Pattern
...
public final class CaseFacade {
    ...
    public CaseDto getCase(String caseId) {
        CaseView view = caseQueryClient.getCase(caseId);
...
```text id="mmmy9d"
Jersey resource tidak boleh membuat ManagedChannel.
Jersey resource tidak sebaiknya tahu generated proto/stub.
```

---

# 29. Testing Cheat Sheet

| Class / Tool | Fungsi |
|---|---|
| `InProcessServerBuilder` | Fake server tanpa port |
| `InProcessChannelBuilder` | Channel tanpa network |
| `grpc-testing` | Utility testing |
| Fake generated service | Simulasi downstream |
| `MetadataUtils.attachHeaders(...)` | Test metadata |
| JUnit/AssertJ/Mockito | Unit test wrapper/mapper |
| Testcontainers | DB/fake infra integration |

Contoh in-process client test:
...
| Test | Expected |
|---|---|
| Success response | Proto mapped ke domain result |
| `NOT_FOUND` | Mapped to `DownstreamNotFoundException` |
| `UNAVAILABLE` | Retry if query/idempotent |
| `DEADLINE_EXCEEDED` | Mapped to timeout |
| Metadata propagation | Correlation/token/tenant sent |
| Token refresh/failure | `UNAUTHENTICATED` handling |
| Channel shutdown | No orphan channel |
| Retry budget | Attempts bounded |
| Idempotency | Same key reused |
| TLS/mTLS | Cert config tested in integration |
...
| Package | Isi |
|---|---|
| `io.grpc` | `Channel`, `ManagedChannel`, `ManagedChannelBuilder`, `Metadata`, `Status`, `ClientInterceptor`, `CallCredentials` |
| `io.grpc.stub` | `AbstractStub`, `StreamObserver`, generated stub base |
| `io.grpc.netty` | `NettyChannelBuilder`, TLS/transport |
| `io.grpc.netty.shaded` | Shaded Netty transport |
| `io.grpc.inprocess` | In-process testing |
| `io.grpc.protobuf` | Protobuf integration, `StatusProto` |
| `com.google.rpc` | Rich error details |
| `com.google.protobuf` | Protobuf runtime |
| generated proto package | `CaseQueryServiceGrpc`, messages, enums |

---

# 31. Performance Practical Rules

gRPC performance best practices menyarankan reuse stubs dan channels bila memungkinkan; untuk Java, docs juga menyarankan non-blocking stubs untuk parallel RPC dan custom executor yang membatasi jumlah thread sesuai workload. ([gRPC](https://grpc.io/docs/guides/performance/))

| Praktik | Alasan |
|---|---|
| Reuse `ManagedChannel` | Channel mahal dan long-lived |
| Reuse base stub | Stub ringan dan immutable-style |
| Jangan channel per request | Socket/thread/resource explosion |
| Deadline wajib | Hindari thread menggantung |
| Blocking stub boleh untuk Jersey blocking flow | Tapi harus ada timeout/concurrency control |
| Async/future untuk fan-out | Lebih baik daripada serial blocking |
| Custom executor | Hindari uncontrolled cached thread pool |
| Channel pool hanya jika high-load dan paham LB | Jangan premature |
| Keepalive hati-hati | Bisa ping storm |
| Compression selektif | CPU vs bandwidth tradeoff |

---

# 32. Anti-Pattern Client

| Anti-Pattern | Dampak |
|---|---|
| Membuat `ManagedChannel` per REST request | Resource explosion |
| Tidak shutdown channel | Thread/socket leak |
| Tidak set deadline | Request bisa wait forever |
| Semua error jadi HTTP 500 | Semantik rusak |
| Retry semua status | Double mutation / retry storm |
| Retry command tanpa idempotency key | Data berubah dua kali |
| Generate idempotency key baru saat retry | Bukan retry idempotent |
| Log full proto payload | PII/security risk |
| Log token metadata | Credential leak |
| `waitForReady` global | Request bisa menumpuk saat outage |
| Keepalive terlalu agresif | Bisa DDoS backend/proxy |
| Plaintext production tanpa policy | Security risk |
| `defaultServiceConfig` retry command global | Dangerous |
| REST resource langsung pakai generated stub | Boundary bocor |
...
| Area | Checklist |
|---|---|
| Channel lifecycle | Channel dibuat saat startup dan shutdown saat stop |
| Channel reuse | Tidak dibuat per request |
| Stub wrapper | Generated stub dibungkus client adapter |
| Deadline | Semua RPC production punya deadline |
| Retry | Method-specific dan idempotency-aware |
| Idempotency | Command mutasi membawa key stabil |
| Metadata | Correlation/auth/tenant/trace dipropagasi |
| Credentials | Auth pakai `CallCredentials` atau interceptor yang aman |
| Security | TLS/mTLS/service mesh policy jelas |
| Error mapping | `StatusRuntimeException` dimap ke domain/application exception |
| Observability | method/status/duration/deadline/retry/correlation visible |
| LB | gRPC-aware LB/service mesh/client-side policy jelas |
| Health | Client health checking jika diperlukan |
| Keepalive | Disepakati dengan service owner |
| Message limits | Max inbound/outbound size dikontrol |
| Testing | In-process, metadata, timeout, retry, security tests tersedia |

---

# 34. Mini Template: Enterprise-Style Config Record
...
# 35. Ringkasan Super Cepat

```text id="czustb"
Java gRPC Client essentials:

- Generated client ada di ServiceNameGrpc.
- Gunakan newBlockingStub, newStub, atau newFutureStub.
- ManagedChannel harus long-lived dan reusable.
- Jangan buat channel per request.
- Selalu set deadline dengan withDeadlineAfter.
- Metadata untuk correlation/auth/tenant/trace.
- Gunakan ClientInterceptor untuk cross-cutting metadata/log/metrics.
- Gunakan CallCredentials untuk per-RPC credentials.
- StatusRuntimeException harus dimap ke application exception.
- Retry hanya method-specific dan idempotency-aware.
- Keepalive jangan agresif.
- TLS/mTLS via NettyChannelBuilder + GrpcSslContexts jika perlu.
- Load balancing gRPC perlu dipikirkan khusus.
- Testing terbaik: InProcessServerBuilder + InProcessChannelBuilder.
```

Rekomendasi materi berikutnya: **Cheat Sheet Java gRPC Service Config & Retry Policy** - khusus membahas JSON service config, retry/hedging/health check/LB policy, deadline budgeting, dan safe rollout di Kubernetes.
...
- [Generated-code reference | Java | gRPC](https://grpc.io/docs/languages/java/generated-code/)
- [ManagedChannel (grpc-all 1.82.0 API)](https://grpc.github.io/grpc-java/javadoc/io/grpc/ManagedChannel.html)
