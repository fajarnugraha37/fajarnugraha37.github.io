...
**Updated:** 7/15/2026 13:36:53  
**Exported:** 7/15/2026 14:39:49  
...
## Response:
7/15/2026, 1:34:43 PM

Saya akan susun sebagai **cheatsheet referensi praktis**: standard Jakarta REST dulu, lalu bagian khusus Jersey untuk konfigurasi/properties yang sering dipakai di microservice.

# Cheatsheet - Jakarta RESTful Web Services: Annotations, Classes, Methods, Properties & Configurations

Referensi ini fokus pada **Jakarta RESTful Web Services 4.0 / Jakarta EE 11 style** dengan package `jakarta.ws.rs.*`. Jakarta REST 4.0 adalah API untuk membangun RESTful web services, dan official API module-nya mengekspor package utama: `jakarta.ws.rs`, `jakarta.ws.rs.client`, `jakarta.ws.rs.container`, `jakarta.ws.rs.core`, `jakarta.ws.rs.ext`, dan `jakarta.ws.rs.sse`. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/module-summary))

---

# 1. Mental Model API Surface

```mermaid
flowchart TD
    A[jakarta.ws.rs] --> A1[Resource annotations]
    A --> A2[HTTP method annotations]
    A --> A3[Param binding annotations]
    A --> A4[Exception hierarchy]

    B[jakarta.ws.rs.core] --> B1[Response / MediaType / UriInfo]
    B --> B2[HttpHeaders / Request / SecurityContext]
    B --> B3[CacheControl / EntityTag / UriBuilder]
    B --> B4[StreamingOutput / EntityPart]

    C[jakarta.ws.rs.container] --> C1[Filters]
    C --> C2[AsyncResponse]
    C --> C3[DynamicFeature]
    C --> C4[ResourceInfo / ResourceContext]

    D[jakarta.ws.rs.ext] --> D1[Provider]
    D --> D2[ExceptionMapper]
    D --> D3[MessageBodyReader / Writer]
    D --> D4[ParamConverterProvider]
    D --> D5[Reader/WriterInterceptor]

    E[jakarta.ws.rs.client] --> E1[ClientBuilder]
    E --> E2[Client / WebTarget]
    E --> E3[Invocation / Entity / Response]
    E --> E4[Client Filters]

    F[jakarta.ws.rs.sse] --> F1[Sse]
    F --> F2[SseEventSink]
    F --> F3[SseBroadcaster]
    F --> F4[SseEventSource]
```
...
# 2. Core Resource Annotations

| Annotation | Package | Fungsi | Contoh |
|---|---|---|---|
| `@ApplicationPath` | `jakarta.ws.rs` | Base URI untuk semua resource dalam aplikasi. | `@ApplicationPath("/api")` |
| `@Path` | `jakarta.ws.rs` | URI path pada class/method. | `@Path("/cases/{caseId}")` |
| `@GET` | `jakarta.ws.rs` | Method menangani HTTP GET. | `@GET` |
| `@POST` | `jakarta.ws.rs` | Method menangani HTTP POST. | `@POST` |
| `@PUT` | `jakarta.ws.rs` | Method menangani HTTP PUT. | `@PUT` |
| `@PATCH` | `jakarta.ws.rs` | Method menangani HTTP PATCH. | `@PATCH` |
| `@DELETE` | `jakarta.ws.rs` | Method menangani HTTP DELETE. | `@DELETE` |
| `@HEAD` | `jakarta.ws.rs` | Method menangani HTTP HEAD. | `@HEAD` |
| `@OPTIONS` | `jakarta.ws.rs` | Method menangani HTTP OPTIONS. | `@OPTIONS` |
| `@HttpMethod` | `jakarta.ws.rs` | Meta-annotation untuk custom HTTP method designator. | `@HttpMethod("PURGE")` |
| `@Consumes` | `jakarta.ws.rs` | Media type request body yang diterima. | `@Consumes("application/json")` |
| `@Produces` | `jakarta.ws.rs` | Media type response yang bisa dihasilkan. | `@Produces("application/json")` |

Official package summary menyebut `@ApplicationPath`, `@Path`, HTTP method annotations, `@Consumes`, dan `@Produces` sebagai high-level API untuk membuat resource REST; spec juga menyatakan resource method adalah method public yang diberi request method designator seperti `@GET`, `@POST`, `@PUT`, `@DELETE`, `@PATCH`, `@HEAD`, dan `@OPTIONS`. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/package-summary))
...
| Annotation | Source HTTP | Use Case |
|---|---|---|
| `@PathParam` | URI template/path segment | `/cases/{caseId}` |
| `@QueryParam` | Query string | `/cases?status=OPEN` |
| `@MatrixParam` | Matrix param pada path segment | `/cases;status=OPEN` |
| `@HeaderParam` | HTTP header | `If-Match`, `Idempotency-Key` |
| `@CookieParam` | Cookie | `ui_locale`, session-like cookie |
| `@FormParam` | `application/x-www-form-urlencoded` body | legacy form submit |
| `@BeanParam` | Aggregator object | group pagination/filter params |
| `@DefaultValue` | Fallback default value | `@DefaultValue("50")` |
| `@Encoded` | Disable automatic decoding | raw encoded path/query value |

Official package summary menjelaskan `@BeanParam` sebagai parameter aggregator, `@DefaultValue` sebagai default untuk metadata request yang di-bind oleh `@PathParam`, `@QueryParam`, `@MatrixParam`, `@CookieParam`, `@FormParam`, atau `@HeaderParam`, dan `@Encoded` untuk menonaktifkan automatic decoding. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/package-summary))
...
public class CaseSearchParams {
    ...
    public CaseSearchCriteria toCriteria() {
        return new CaseSearchCriteria(statuses, page, size);
    }
}
```
...
| Context | Package | Fungsi Praktis |
|---|---|---|
| `@Context` | `jakarta.ws.rs.core` | Inject runtime/request context. |
| `UriInfo` | `jakarta.ws.rs.core` | URI, base URI, request URI, query/path params. |
| `HttpHeaders` | `jakarta.ws.rs.core` | Header, cookie, Accept, Content-Type. |
| `Request` | `jakarta.ws.rs.core` | Conditional request: ETag, Last-Modified. |
| `SecurityContext` | `jakarta.ws.rs.core` | Principal, role, secure channel, auth scheme. |
| `Application` | `jakarta.ws.rs.core` | App-level JAX-RS metadata. |
| `Configuration` | `jakarta.ws.rs.core` | Runtime config/properties/provider registration state. |
| `Providers` | `jakarta.ws.rs.ext` | Lookup provider runtime: mapper, reader, writer, resolver. |
| `ResourceContext` | `jakarta.ws.rs.container` | Access/initialize resource/sub-resource instances. |
| `ResourceInfo` | `jakarta.ws.rs.container` | Matched resource class/method; useful in filters/features. |

`@Context` digunakan untuk inject informasi ke field, property, atau method parameter; `HttpHeaders`, `Request`, `SecurityContext`, dan `UriInfo` adalah injectable interfaces untuk headers, request processing, security, dan URI information. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/core/package-summary))
...
public Response getCase(
    ...
    CaseResponse response = caseService.getCase(actor, caseId);
...
# 5. Core Classes: `jakarta.ws.rs.core`

| Class/Interface | Penting Method/Property | Use Case |
|---|---|---|
| `Application` | `getClasses()`, `getSingletons()`, `getProperties()` | Deklarasi komponen JAX-RS. |
| `Response` | `ok()`, `status()`, `created()`, `accepted()`, `noContent()`, `entity()`, `header()`, `tag()`, `cacheControl()`, `build()` | Build HTTP response. |
| `Response.ResponseBuilder` | `entity`, `type`, `location`, `links`, `replaceAll`, `build` | Fluent response metadata. |
| `Response.Status` | `OK`, `CREATED`, `ACCEPTED`, `NO_CONTENT`, `BAD_REQUEST`, etc. | Status code standard. |
| `MediaType` | `APPLICATION_JSON`, `TEXT_PLAIN`, `SERVER_SENT_EVENTS`, `valueOf()` | Media type constants. |
| `HttpHeaders` | `getHeaderString`, `getRequestHeaders`, `getAcceptableMediaTypes`, `getCookies`, `getMediaType` | Baca header. |
| `UriInfo` | `getBaseUri`, `getRequestUri`, `getAbsolutePath`, `getPath`, `getQueryParameters`, `getPathParameters`, `getAbsolutePathBuilder` | URI/link building. |
| `UriBuilder` | `fromUri`, `path`, `queryParam`, `resolveTemplate`, `build` | Safe URI construction. |
| `Request` | `evaluatePreconditions(EntityTag)`, `evaluatePreconditions(Date)`, `selectVariant()` | ETag/conditional requests. |
| `SecurityContext` | `getUserPrincipal`, `isUserInRole`, `isSecure`, `getAuthenticationScheme` | Identity/role bridge. |
| `CacheControl` | `setNoStore`, `setNoCache`, `setMaxAge`, `setPrivate` | Cache-Control header. |
| `EntityTag` | constructor, `value`, `isWeak` | ETag header. |
| `GenericEntity<T>` | wraps generic response entity | Return `List<T>` with generic type. |
| `GenericType<T>` | captures generic type | Client read `List<T>`. |
| `MultivaluedMap<K,V>` | `getFirst`, `add`, `putSingle` | Headers/query/path params. |
| `Cookie` / `NewCookie` | cookie request/response model | Cookie handling. |
| `Link` | `fromUri`, `rel`, `build` | Hypermedia link. |
| `Variant` | media/language/encoding variant | Content negotiation. |
| `StreamingOutput` | `write(OutputStream)` | Streaming download/export. |
| `EntityPart` | multipart part model | Standard multipart part. |
| `PathSegment` | `getPath`, `getMatrixParameters` | Matrix params per path segment. |

Official `jakarta.ws.rs.core` summary lists these low-level APIs, including `Response`, `ResponseBuilder`, `MediaType`, `HttpHeaders`, `UriInfo`, `SecurityContext`, `EntityTag`, `CacheControl`, `EntityPart`, `StreamingOutput`, `UriBuilder`, `MultivaluedMap`, and related classes. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/core/package-summary))

---

# 6. Response Builder Quick Cheatsheet

```java
Response.ok(entity).build();                       // 200
Response.created(location).entity(entity).build(); // 201 + Location
Response.accepted(operation).location(uri).build(); // 202
Response.noContent().build();                      // 204
Response.status(400).entity(problem).build();      // custom status
```
...
| Exception | Typical HTTP | Use Case |
|---|---:|---|
| `WebApplicationException` | custom | Base runtime exception with response/status. |
| `BadRequestException` | `400` | Malformed/invalid client request. |
| `NotAuthorizedException` | `401` | Authentication required/failed. |
| `ForbiddenException` | `403` | Authenticated but forbidden. |
| `NotFoundException` | `404` | Resource not found. |
| `NotAllowedException` | `405` | Method not allowed. |
| `NotAcceptableException` | `406` | Cannot produce acceptable media type. |
| `NotSupportedException` | `415` | Unsupported request media type. |
| `ClientErrorException` | `4xx` | Base client error exception. |
| `ServerErrorException` | `5xx` | Base server error exception. |
| `InternalServerErrorException` | `500` | Internal error. |
| `ServiceUnavailableException` | `503` | Service unavailable. |
| `ProcessingException` | N/A | Runtime processing failure. |
| `RedirectionException` | `3xx` | Redirection response. |

Jakarta REST package summary lists the exception hierarchy and describes common status-specific exceptions such as `BadRequestException`, `ForbiddenException`, `NotAcceptableException`, `NotAllowedException`, `NotAuthorizedException`, `NotFoundException`, `NotSupportedException`, and `ServiceUnavailableException`. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/package-summary))
...
# 8. Filters & Container API

| API | Method Utama | Use Case |
|---|---|---|
| `ContainerRequestFilter` | `filter(ContainerRequestContext ctx)` | Auth, correlation ID, rate limit, request validation. |
| `ContainerResponseFilter` | `filter(ContainerRequestContext req, ContainerResponseContext res)` | Security headers, metrics, response logging. |
| `ContainerRequestContext` | `getUriInfo`, `getHeaders`, `getHeaderString`, `getMethod`, `setMethod`, `abortWith`, `getSecurityContext`, `setSecurityContext`, `setProperty`, `getProperty`, `hasEntity`, `getEntityStream`, `setEntityStream` | Mutable request context in filters. |
| `ContainerResponseContext` | `getStatus`, `setStatus`, `getHeaders`, `setEntity`, `hasEntity`, `getMediaType` | Mutable response context. |
| `@PreMatching` | annotation | Request filter before resource matching. |
| `DynamicFeature` | `configure(ResourceInfo, FeatureContext)` | Register filters/providers per resource method. |
| `ResourceInfo` | `getResourceClass`, `getResourceMethod` | Inspect matched resource. |
| `AsyncResponse` | `resume`, `setTimeout`, `setTimeoutHandler`, `register`, `cancel`, `isDone`, `isSuspended` | Async/long polling. |
| `@Suspended` | annotation | Inject suspended `AsyncResponse`. |
| `TimeoutHandler` | `handleTimeout(AsyncResponse)` | Async timeout response. |
| `CompletionCallback` | `onComplete(Throwable)` | Request completion callback. |
| `ConnectionCallback` | `onDisconnect(AsyncResponse)` | Client disconnect callback. |

Official container API summary lists `AsyncResponse`, `ContainerRequestFilter`, `ContainerResponseFilter`, `DynamicFeature`, `PreMatching`, `ResourceContext`, `ResourceInfo`, `Suspended`, and `TimeoutHandler`. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/container/package-summary))
    ...
    public void filter(ContainerRequestContext ctx) {
        ...
        if (authorization == null || !authorization.startsWith("Bearer ")) {
            ctx.abortWith(Response.status(Response.Status.UNAUTHORIZED)
                ...
                .type("application/problem+json")
                .entity(ProblemDetail.authenticationRequired())
                .build());
            return;
        }

        Actor actor = tokenVerifier.verify(authorization);
        ctx.setProperty("actor", actor);
...
# 9. Provider & Extension API

| API | Method Utama | Use Case |
|---|---|---|
| `@Provider` | annotation | Mark provider discoverable by runtime. |
| `ExceptionMapper<E>` | `toResponse(E exception)` | Exception → `Response`. |
| `MessageBodyReader<T>` | `isReadable`, `readFrom` | Request body stream → Java object. |
| `MessageBodyWriter<T>` | `isWriteable`, `writeTo` | Java object → response stream. |
| `ParamConverter<T>` | `fromString`, `toString` | String param ↔ custom Java type. |
| `ParamConverterProvider` | `getConverter(rawType, genericType, annotations)` | Provide converters. |
| `ContextResolver<T>` | `getContext(Class<?> type)` | Provide context object, e.g. JSON mapper config. |
| `Providers` | `getMessageBodyReader`, `getMessageBodyWriter`, `getExceptionMapper`, `getContextResolver` | Runtime provider lookup. |
| `ReaderInterceptor` | `aroundReadFrom` | Wrap request entity read. |
| `WriterInterceptor` | `aroundWriteTo` | Wrap response entity write. |
| `Feature` | `configure(FeatureContext)` | Bundle registration/config. |
| `FeatureContext` | `register`, `property` | Provider registration/config in feature. |

Official extension package summary lists `ContextResolver`, `ExceptionMapper`, `MessageBodyReader`, `MessageBodyWriter`, `ParamConverter`, `ParamConverterProvider`, `Provider`, `Providers`, `ReaderInterceptor`, and `WriterInterceptor`. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/ext/package-summary))

Example custom `ParamConverterProvider`:
...
| API | Package | Fungsi |
|---|---|---|
| `@Priority` | `jakarta.annotation` | Order filter/interceptor/provider. |
| `Priorities` | `jakarta.ws.rs` | Built-in priority constants. |
| `@NameBinding` | `jakarta.ws.rs` | Bind filter/interceptor ke resource tertentu. |
| `@ConstrainedTo` | `jakarta.ws.rs` | Batasi provider untuk `RuntimeType.CLIENT` atau `SERVER`. |
| `RuntimeType` | `jakarta.ws.rs` | `CLIENT` / `SERVER`. |

`Priorities` menyediakan built-in priority constants untuk komponen yang diurutkan berdasarkan `jakarta.annotation.Priority`, dan `NameBinding` adalah meta-annotation untuk membuat annotation binding filter/interceptor. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/package-summary))
...
| API | Method Penting | Use Case |
|---|---|---|
| `ClientBuilder` | `newClient`, `newBuilder`, `connectTimeout`, `readTimeout`, `sslContext`, `trustStore`, `keyStore`, `hostnameVerifier`, `executorService`, `scheduledExecutorService`, `build` | Build configured client. |
| `Client` | `target`, `register`, `property`, `close`, `getConfiguration` | Reusable client root. |
| `WebTarget` | `path`, `queryParam`, `resolveTemplate`, `request`, `getUri` | Build target URI. |
| `Invocation.Builder` | `header`, `accept`, `type`, `get`, `post`, `put`, `delete`, `method`, `buildGet`, `buildPost`, `async`, `rx` | Build/execute request. |
| `Entity<T>` | `json`, `xml`, `text`, `entity` | Request body wrapper. |
| `Response` | `getStatus`, `getStatusInfo`, `hasEntity`, `readEntity`, `getHeaderString`, `close` | Raw response handling. |
| `ClientRequestFilter` | `filter(ClientRequestContext)` | Add auth/correlation headers. |
| `ClientResponseFilter` | `filter(ClientRequestContext, ClientResponseContext)` | Log/metric outbound response. |
| `AsyncInvoker` | async HTTP methods | Async client calls. |
| `InvocationCallback<T>` | `completed`, `failed` | Async callback. |
| `CompletionStageRxInvoker` | reactive invocation | `CompletionStage` style. |
| `GenericType<T>` | captured generic response | Read `List<Foo>`. |

Official client API summary states `ClientBuilder` is the main entry point to bootstrap `Client` instances, `Client` is the heavy-weight configurable root object, and the fluent flow is `Client` → `WebTarget` → request builder → invocation/response. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/client/package-summary))
...
WebTarget target = client.target("https://risk.internal/api");

try (Response response = target.path("risk-scores")
        .request(MediaType.APPLICATION_JSON_TYPE)
        .header("Idempotency-Key", idempotencyKey)
        .post(Entity.json(request))) {
    ...
    throw remoteProblemMapper.toException(response);
}
```
...
| API | Method/Property Penting | Use Case |
|---|---|---|
| `Sse` | `newEvent`, `newEventBuilder`, `newBroadcaster` | Create outbound events/broadcaster. |
| `SseEventSink` | `send`, `isClosed`, `close` | Server-side SSE stream to one client. |
| `OutboundSseEvent` | `id`, `name`, `data`, `reconnectDelay`, `comment` | One outbound event. |
| `SseBroadcaster` | `register`, `broadcast`, `close` | Broadcast to many sinks. |
| `SseEventSource` | `target`, `register`, `open`, `close` | Client-side SSE consumer. |
| `InboundSseEvent` | `getName`, `readData`, `getId`, `getReconnectDelay` | Incoming event on client. |

Official SSE package summary says `jakarta.ws.rs.sse` supports server event streams and client-side processing; it lists `Sse`, `SseEventSink`, `SseBroadcaster`, `SseEventSource`, `OutboundSseEvent`, and `InboundSseEvent`. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/sse/package-summary))
...
public void events(
    ...
    operationEventService.subscribe(operationId, event -> {
        if (!sink.isClosed()) {
            sink.send(sse.newEventBuilder()
                .id(event.sequenceId())
                .name(event.type())
                .data(OperationEventDto.class, event.toDto())
                .reconnectDelay(3000)
                .build());
        }
    });
...
# 13. Java SE Bootstrap

| API | Fungsi |
|---|---|
| `SeBootstrap` | Start Jakarta REST app di Java SE environment. |
| `SeBootstrap.Configuration` | Host, port, protocol, root path, SSL, properties. |
| `SeBootstrap.Configuration.Builder` | Builder config. |
| `SeBootstrap.Instance` | Running app handle. |

`SeBootstrap` adalah bootstrap class untuk menjalankan aplikasi JAX-RS di Java SE environment. ([Jakarta EE](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/jakarta/ws/rs/package-summary))

Example conceptual:

```java
Application app = new ApiApplication();
...
CompletionStage<SeBootstrap.Instance> instance =
    SeBootstrap.start(app, config);
```

---

# 14. Minimal Standard App Setup
...
public class ApiApplication extends Application {
    ...
    public Set<Class<?>> getClasses() {
        return Set.of(
            ...
            CaseIdParamConverterProvider.class
        );
    }
}
```

Dengan Jersey biasanya lebih praktis memakai `ResourceConfig`.

---

# 15. Jersey `ResourceConfig` Setup

| Method | Fungsi |
|---|---|
| `packages(String...)` | Scan package resource/provider. |
| `register(Class<?>)` | Register resource/provider/filter/feature class. |
| `register(Object)` | Register singleton instance. |
| `property(String, Object)` | Set config property. |
| `setProperties(Map)` | Replace all properties. |
| `addProperties(Map)` | Add/override properties. |
| `getClasses()` | Registered classes. |
| `getProperties()` | Current properties. |

Jersey `ResourceConfig` supports adding/replacing properties and registering component classes; `property(...)` comes from `Configurable`. ([Eclipse EE4J](https://eclipse-ee4j.github.io/jersey.github.io/apidocs/3.1.10/jersey/org/glassfish/jersey/server/ResourceConfig.html))
...
public class ApiApplication extends ResourceConfig {
    ...
    public ApiApplication() {
        ...
        property("jersey.config.server.application.name", "case-api");
...
# 16. Jersey Common Properties

Ini **bukan Jakarta REST standard**, tetapi sering dipakai kalau implementasinya Jersey.

| Property | Constant | Fungsi |
|---|---|---|
| `jersey.config.disableAutoDiscovery` | `CommonProperties.FEATURE_AUTO_DISCOVERY_DISABLE` | Disable auto discovery global client/server. |
| `jersey.config.disableJsonProcessing` | `CommonProperties.JSON_PROCESSING_FEATURE_DISABLE` | Disable JSON-P auto config. |
| `jersey.config.disableMetainfServicesLookup` | `CommonProperties.METAINF_SERVICES_LOOKUP_DISABLE` | Disable `META-INF/services` lookup. |
| `jersey.config.disableMoxyJson` | `CommonProperties.MOXY_JSON_FEATURE_DISABLE` | Disable MOXy JSON auto feature. |
| `jersey.config.contentLength.buffer` | `CommonProperties.OUTBOUND_CONTENT_LENGTH_BUFFER` | Buffer size untuk hitung `Content-Length`. |
| `jersey.config.logging.logger.name` | `LoggingFeature.LOGGING_FEATURE_LOGGER_NAME` | Logger name. |
| `jersey.config.logging.logger.level` | `LoggingFeature.LOGGING_FEATURE_LOGGER_LEVEL` | Logging level. |
| `jersey.config.logging.verbosity` | `LoggingFeature.LOGGING_FEATURE_VERBOSITY` | `HEADERS_ONLY`, `PAYLOAD_TEXT`, `PAYLOAD_ANY`. |
| `jersey.config.logging.entity.maxSize` | `LoggingFeature.LOGGING_FEATURE_MAX_ENTITY_SIZE` | Max entity bytes logged. |

Jersey documentation lists these common properties in `CommonProperties`; auto-discovery, JSON processing, META-INF services lookup, MOXy JSON, content length buffer, and logging feature properties are included. ([Eclipse EE4J](https://eclipse-ee4j.github.io/jersey.github.io/documentation/2.29.1/appendix-properties.html))

Production rule:

```text
Do not enable payload logging broadly in production.
Use HEADERS_ONLY or structured application logging with redaction.
```

---

# 17. Jersey Server Properties

| Property | Constant | Fungsi |
|---|---|---|
| `jersey.config.server.application.name` | `ServerProperties.APPLICATION_NAME` | Application name, useful for monitoring/JMX. |
| `jersey.config.beanValidation.disable.server` | `ServerProperties.BV_FEATURE_DISABLE` | Disable Bean Validation support. |
| `jersey.config.beanValidation.enableOutputValidationErrorEntity.server` | `ServerProperties.BV_SEND_ERROR_IN_RESPONSE` | Send validation error info in response. |
| `jersey.config.server.disableAutoDiscovery` | `ServerProperties.FEATURE_AUTO_DISCOVERY_DISABLE` | Disable server auto discovery. |
| `jersey.config.server.httpMethodOverride` | `ServerProperties.HTTP_METHOD_OVERRIDE` | Configure HTTP method override filter behavior. |
| `jersey.config.server.disableJsonProcessing` | `ServerProperties.JSON_PROCESSING_FEATURE_DISABLE` | Disable JSON-P feature. |
| `jersey.config.server.languageMappings` | `ServerProperties.LANGUAGE_MAPPINGS` | URI extension → language mapping. |
| `jersey.config.server.mediaTypeMappings` | `ServerProperties.MEDIA_TYPE_MAPPINGS` | URI extension → media type mapping. |
| `jersey.config.server.disableMetainfServicesLookup` | `ServerProperties.METAINF_SERVICES_LOOKUP_DISABLE` | Disable server `META-INF/services` lookup. |
| `jersey.config.server.disableMoxyJson` | `ServerProperties.MOXY_JSON_FEATURE_DISABLE` | Disable MOXy JSON. |
| `jersey.config.server.monitoring.enabled` | `ServerProperties.MONITORING_ENABLED` | Enable monitoring. |
| `jersey.config.server.monitoring.statistics.enabled` | `ServerProperties.MONITORING_STATISTICS_ENABLED` | Enable monitoring statistics. |
| `jersey.config.server.monitoring.statistics.mbeans.enabled` | `ServerProperties.MONITORING_STATISTICS_MBEANS_ENABLED` | Expose monitoring stats as JMX MBeans. |
| `jersey.config.server.monitoring.statistics.refresh.interval` | `ServerProperties.MONITORING_STATISTICS_REFRESH_INTERVAL` | Stats refresh interval in ms. |
| `jersey.config.contentLength.server.buffer` | `ServerProperties.OUTBOUND_CONTENT_LENGTH_BUFFER` | Server outbound content length buffer. |

Jersey docs list these server properties in `ServerProperties`; monitoring/statistics properties can have performance impact and should be enabled only when needed. ([Eclipse EE4J](https://eclipse-ee4j.github.io/jersey.github.io/documentation/2.29.1/appendix-properties.html))

---

# 18. Jersey Servlet Properties

| Property | Constant | Fungsi |
|---|---|---|
| `jersey.config.servlet.filter.contextPath` | `ServletProperties.FILTER_CONTEXT_PATH` | Jersey servlet filter context path. |
| `jersey.config.servlet.filter.forwardOn404` | `ServletProperties.FILTER_FORWARD_ON_404` | Forward empty 404 to next servlet filter. |
| `jersey.config.servlet.filter.staticContentRegex` | `ServletProperties.FILTER_STATIC_CONTENT_REGEX` | Static content handled by servlet engine. |
| `javax.ws.rs.Application` | `ServletProperties.JAXRS_APPLICATION_CLASS` | FQCN of JAX-RS `Application` class. |
| `jersey.config.servlet.provider.webapp` | `ServletProperties.PROVIDER_WEB_APP` | Scan whole web app for resources/providers. |
| `jersey.config.servlet.form.queryParams.disabled` | `ServletProperties.QUERY_PARAMS_AS_FORM_PARAMS_DISABLED` | Query params not treated as form params. |
| `jersey.config.servlet.context.serviceLocator` | `ServletProperties.SERVICE_LOCATOR` | Parent HK2 ServiceLocator. |

Jersey servlet properties are documented separately in `ServletProperties`, including filter context path, forward-on-404, static content regex, application class init property, and whole-webapp provider scanning. ([Eclipse EE4J](https://eclipse-ee4j.github.io/jersey.github.io/documentation/2.29.1/appendix-properties.html))

---

# 19. Jersey Client Properties

| Property | Constant | Fungsi |
|---|---|---|
| `jersey.config.client.connectTimeout` | `ClientProperties.CONNECT_TIMEOUT` | Connect timeout ms. |
| `jersey.config.client.readTimeout` | `ClientProperties.READ_TIMEOUT` | Read timeout ms. |
| `jersey.config.client.followRedirects` | `ClientProperties.FOLLOW_REDIRECTS` | Follow 3xx redirects. |
| `jersey.config.client.proxy.uri` | `ClientProperties.PROXY_URI` | HTTP proxy URI. |
...
| `jersey.config.client.request.entity.processing` | `ClientProperties.REQUEST_ENTITY_PROCESSING` | `BUFFERED` or `CHUNKED`. |
| `jersey.config.client.chunkedEncodingSize` | `ClientProperties.CHUNKED_ENCODING_SIZE` | Chunked encoding size. |
| `jersey.config.client.disableAutoDiscovery` | `ClientProperties.FEATURE_AUTO_DISCOVERY_DISABLE` | Disable client auto-discovery. |
| `jersey.config.client.disableJsonProcessing` | `ClientProperties.JSON_PROCESSING_FEATURE_DISABLE` | Disable JSON-P feature. |
| `jersey.config.client.disableMoxyJson` | `ClientProperties.MOXY_JSON_FEATURE_DISABLE` | Disable MOXy JSON. |
| `jersey.config.client.suppressHttpComplianceValidation` | `ClientProperties.SUPPRESS_HTTP_COMPLIANCE_VALIDATION` | Suppress strict HTTP compliance validation. |
| `jersey.config.client.useEncoding` | `ClientProperties.USE_ENCODING` | Content-Encoding for `EncodingFilter`. |
| `jersey.config.client.logging.logger.name` | Logging feature client property | Client logging logger name. |
| `jersey.config.client.logging.logger.level` | Logging feature client property | Client logging level. |
| `jersey.config.client.logging.verbosity` | Logging feature client property | Client logging verbosity. |

Jersey docs list `ClientProperties` including async pool size, chunked encoding, connect/read timeout, redirect, proxy settings, request entity processing, suppress HTTP compliance validation, encoding, and logging properties; read/connect timeout defaults are `0` / infinity in the referenced docs, so production clients should set explicit values. ([Eclipse EE4J](https://eclipse-ee4j.github.io/jersey.github.io/documentation/2.29.1/appendix-properties.html))
...
Client client = ClientBuilder.newBuilder()
    ...
    .property("jersey.config.client.request.entity.processing", "CHUNKED")
    .register(JacksonFeature.class)
    .build();
```
...
# 20. Common Media Type Constants
...
| `MediaType.SERVER_SENT_EVENTS` | `text/event-stream` |
| `MediaType.WILDCARD` | `*/*` |
...
# 23. Important Lifecycle Hooks by Concern

| Concern | Jakarta REST API |
|---|---|
| Resource routing | `@ApplicationPath`, `@Path`, HTTP method annotations |
| Request params | `@PathParam`, `@QueryParam`, `@HeaderParam`, `@BeanParam` |
| Request body | `MessageBodyReader`, entity parameter |
| Response body | `MessageBodyWriter`, return object / `Response` |
| Error mapping | `ExceptionMapper` |
| Authentication | `ContainerRequestFilter`, `SecurityContext` |
| Authorization | request filter, `SecurityContext`, domain service |
| Correlation ID | request + response filters |
| Conditional GET | `Request`, `EntityTag`, `CacheControl` |
| Upload multipart | `EntityPart` standard or Jersey multipart feature |
| Streaming download | `StreamingOutput` |
| Async response | `@Suspended AsyncResponse` |
| SSE | `Sse`, `SseEventSink`, `SseBroadcaster` |
| Client call | `ClientBuilder`, `Client`, `WebTarget`, `Invocation.Builder` |
| Client cross-cutting | `ClientRequestFilter`, `ClientResponseFilter` |
| Custom param type | `ParamConverterProvider` |
| Custom JSON config | `ContextResolver<ObjectMapper>` or JSON-B resolver/provider |
| Per-resource provider binding | `@NameBinding`, `DynamicFeature` |

---

# 24. Production Registration Template
...
public class ApiApplication extends ResourceConfig {
    ...
    public ApiApplication() {
        ...
        // Converters/providers
        register(CaseIdParamConverterProvider.class);

        // Features
        register(IdempotencyFeature.class);
        register(AuditFeature.class);

        // Jersey properties
        property("jersey.config.server.application.name", "case-api");
        property("jersey.config.server.disableMoxyJson", true);
        property("jersey.config.server.disableAutoDiscovery", false);
    }
}
```
...
# 25. Quick “What Should I Use?” Table

| Kebutuhan | Gunakan |
|---|---|
| Membuat endpoint | `@Path`, `@GET`, `@POST`, `@Consumes`, `@Produces` |
| Ambil ID dari URL | `@PathParam` |
| Filter/search/pagination | `@QueryParam`, `@BeanParam` |
| Header `If-Match` / `Idempotency-Key` | `@HeaderParam` or filter |
| Build `201 Location` | `UriInfo.getAbsolutePathBuilder()` |
| Return custom status/header/body | `Response` |
| Conditional GET | `Request.evaluatePreconditions`, `EntityTag` |
| Custom error response | `ExceptionMapper` |
| Auth filter | `ContainerRequestFilter` |
| Security principal | `SecurityContext` |
| Body serialization | `MessageBodyReader` / `MessageBodyWriter` |
| Custom param type | `ParamConverterProvider` |
| Stream response | `StreamingOutput` |
| Multipart part | `EntityPart` or Jersey multipart |
| Long polling | `@Suspended AsyncResponse` |
| SSE | `Sse`, `SseEventSink` |
| Outbound HTTP | `ClientBuilder`, `Client`, `WebTarget` |
| Outbound auth/correlation | `ClientRequestFilter` |
| Jersey app config | `ResourceConfig.property(...)` |

---

# 26. Anti-Pattern Checklist

| Anti-pattern | Hindari Karena |
|---|---|
| Semua endpoint return raw entity tanpa `Response` saat butuh headers/status | Sulit set `Location`, `ETag`, `Cache-Control`. |
| Business logic di resource class | Layering buruk, sulit test. |
| Auth parsing di setiap resource method | Duplikasi dan rawan security bug. |
| No explicit `@Consumes/@Produces` | 406/415 surprise. |
| Mix `javax.ws.rs.*` dan `jakarta.ws.rs.*` | Runtime/provider mismatch. |
| Per-request `ClientBuilder.newClient()` | Connection/resource leak. |
| Raw `Response` client tidak ditutup | Pool leak. |
| Multiple JSON providers tanpa policy | Serialization drift. |
| Payload logging di production | PII/secret leak. |
| `@Context` object dipassing ke async worker | Request scope invalid. |

---

# 27. Roadmap Hafalan Cepat

1. **Resource annotations**: `@Path`, HTTP methods, `@Consumes`, `@Produces`.
2. **Parameter binding**: `@PathParam`, `@QueryParam`, `@HeaderParam`, `@BeanParam`.
3. **Core context**: `UriInfo`, `HttpHeaders`, `Request`, `SecurityContext`.
4. **Response building**: `Response`, `MediaType`, `EntityTag`, `CacheControl`.
5. **Providers**: `ExceptionMapper`, `MessageBodyReader/Writer`, `ParamConverterProvider`.
6. **Filters/interceptors**: request/response filter, reader/writer interceptor, priority, name binding.
7. **Client API**: `ClientBuilder`, `Client`, `WebTarget`, `Entity`, `Response`.
8. **Jersey config**: `ResourceConfig`, JSON provider, server/client properties.
9. **Async/SSE**: `AsyncResponse`, `Sse`, `SseEventSink`.
10. **Troubleshooting**: 404/405/406/415/500 mapped to route/method/media/provider/runtime issues.

Rekomendasi berikutnya: **cheatsheet “Jakarta REST/Jersey code snippets cookbook”** berisi template siap pakai untuk resource, filter, exception mapper, DTO validation, Jersey client, SSE, upload/download, dan tests.

---

**Sources:**

- [jakarta.ws.rs (Jakarta RESTful WS API 4.0.0 API)](https://jakarta.ee/specifications/restful-ws/4.0/apidocs/jakarta.ws.rs/module-summary)
- [ResourceConfig (Jersey 3.1.10 API)](https://eclipse-ee4j.github.io/jersey.github.io/apidocs/3.1.10/jersey/org/glassfish/jersey/server/ResourceConfig.html)
